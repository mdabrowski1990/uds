...
def validate_single_frame_data(addressing_format: CanAddressingFormat, raw_frame_data: RawBytesAlias) -> None:
    ...
    if dlc <= MAX_DLC_VALUE_SHORT_SF_DL:
        ...
        if sf_dl_data_bytes[0] & 0xF != 0:
            raise InconsistencyError(
                "Value of Single Frame Data Length must use 0x00 at the first byte when "
    ...
    if min_dlc < dlc:
        ...
        if dlc > CanDlcHandler.MIN_BASE_UDS_DLC:
            warn(
                message=f"DLC greater than {CanDlcHandler.MIN_BASE_UDS_DLC} is used for CAN Packets "
                f"without data padding which unnecessarily increases the bus load.",
                category=ValueWarning,
            )


def create_single_frame_data(
    ...
    frame_dlc = (
        get_single_frame_min_dlc(addressing_format=addressing_format, payload_length=len(payload))
        if dlc is None
        else dlc
    )
    frame_data_bytes_number = CanDlcHandler.decode_dlc(frame_dlc)
    sf_dl_bytes = encode_sf_dl(sf_dl=len(payload), dlc=frame_dlc, addressing_format=addressing_format)
    sf_bytes = ai_data_bytes + sf_dl_bytes + bytearray(payload)
    if len(sf_bytes) > frame_data_bytes_number:
        raise InconsistencyError(
            "Provided value of `payload` contains of too many bytes to fit in. Consider increasing DLC value."
        )
...
def generate_single_frame_data(
    addressing_format: CanAddressingFormat,
    payload: RawBytesAlias,
    dlc: int,
    sf_dl_short: int,
...
def get_max_sf_dl(addressing_format: CanAddressingFormat, dlc: int | None = None) -> int:
    """
    ...
    ai_data_bytes_number = (
        0 if addressing_format is None else CanAddressingInformation.get_ai_data_bytes_number(addressing_format)
    )
    output = frame_data_bytes_number - ai_data_bytes_number - sf_dl_bytes_number
    if output <= 0:
        raise InconsistencyError(
            "Provided values cannot be used to transmit a valid Single Frame packet. "
...
def extract_sf_dl_data_bytes(addressing_format: CanAddressingFormat, raw_frame_data: RawBytesAlias) -> bytearray:
    """
...
def encode_sf_dl(addressing_format: CanAddressingFormat, dlc: int, sf_dl: int) -> bytearray:
    """
...
def validate_sf_dl(addressing_format: CanAddressingFormat, dlc: int, sf_dl: int) -> None:
    """
    ...
    if sf_dl > max_sf:
        raise InconsistencyError(
            "Provided value of `sf_dl` is greater than maximum valid value of Single Frame "
