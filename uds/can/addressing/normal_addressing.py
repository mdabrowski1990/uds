    ...
    def validate_addressing_params(
        ...
        if (target_address, source_address, address_extension) != (None, None, None):
            raise UnusedArgumentError(
                "Values of Target Address, Source Address and Address Extension are "
...
class NormalFixedCanAddressingInformation(AbstractCanAddressingInformation):
    ...
    def _validate_addressing_information(self) -> None:
        ...
        if (
            ...
            raise InconsistencyError(
                "Target Address and Source Address for incoming physically addressed "
    ...
    def validate_addressing_params(
        ...
        if address_extension is not None:
            raise UnusedArgumentError(
                "Values ofAddress Extension is not supported by Normal Fixed Addressing format and must be equal None."
            )
        addressing_type = AddressingType.validate_member(addressing_type)
        if can_id is None:
            if None in (target_address, source_address):
                raise InconsistencyError(
                    "Values of target_address and source_address must be provided, "
    ...
    def is_compatible_can_id(can_id: int, addressing_type: AddressingType | None = None) -> bool:
        ...
        if masked_can_id == CanIdHandler.NORMAL_FIXED_PHYSICAL_ADDRESSING_MASKED_VALUE and addressing_type in {
            None,
            AddressingType.PHYSICAL,
        }:
            return True
        if masked_can_id == CanIdHandler.NORMAL_FIXED_FUNCTIONAL_ADDRESSING_MASKED_VALUE and addressing_type in {
            None,
