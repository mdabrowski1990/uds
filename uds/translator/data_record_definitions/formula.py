...
from .subfunctions import EVENT_TYPE_2013, EVENT_TYPE_2020, REPORT_TYPE_2020

# Shared


def get_raw_data_record_with_length_formula(
    data_record_name: str, accept_zero_length: bool
    ...
    def get_raw_data_record(length: int) -> tuple[RawDataRecord] | tuple[()]:
        ...
        raise ValueError(
            "Unexpected length value provided. "
...
def get_did_data_2020(name: str = "DID data") -> ConditionalFormulaDataRecord:
    ...
    def _get_did_data(did: int) -> tuple[RawDataRecord]:
        ...
        return (
            RawDataRecord(name=name, children=data_records, length=total_length, min_occurrences=1, max_occurrences=1),
        )
...
def get_did_record_2020(
    did_count: int, record_number: int | None, optional: bool = False
...
def get_memory_size_and_memory_address(
    ...
    if memory_address_length == 0 or memory_size_length == 0:
        raise ValueError(
            "Provided `addressAndLengthFormatIdentifier` value "
    ...
    return (
        RawDataRecord(name="memoryAddress", length=8 * memory_address_length),
...
def get_max_number_of_block_length(length_format_identifier: int) -> tuple[RawDataRecord]:
    ...
    if bytes_number == 0:
        raise ValueError(
            f"Provided `lengthFormatIdentifier` value (0x{length_format_identifier:02X}) is incorrect "
...
def get_did_records_formula_2020(record_number: int | None) -> Callable[[int], MessageStructureAlias]:
    ...
    """
    return lambda did_count: get_did_record_2020(did_count=did_count, record_number=record_number)
...
def get_did_records_formula_2013(record_number: int | None) -> Callable[[int], MessageStructureAlias]:
    ...
    :return: Formula for given record (e.g. Snapshot or Stored Data).
    """
    return lambda did_count: get_did_record_2013(did_count=did_count, record_number=record_number)


# SID 0x24


def get_scaling_byte_extension(
    scaling_byte: int, scaling_byte_number: int
) -> tuple[RawDataRecord | ConditionalFormulaDataRecord, ...]:
    """
    ...
    if parameter_type == 0x2:  # bitMappedReportedWithOutMask
        ...
        return (
            RawDataRecord(
                ...
                children=(RawDataRecord(name="validityMask", length=8 * number_of_bytes),),
            ),
        )
...
def get_scaling_byte_extension_formula(scaling_byte_number: int) -> Callable[[int], MessageStructureAlias]:
    ...
    """
    return lambda scaling_byte: get_scaling_byte_extension(
        scaling_byte=scaling_byte, scaling_byte_number=scaling_byte_number
    )
...
def get_coefficients(formula_identifier: int, scaling_byte_number: int) -> tuple[CustomFormulaDataRecord, ...]:
    ...
    if isinstance(physical_value, str) and "C0" in physical_value:
        data_records = []
        constant_index = 0
        encoding_formula = get_encode_float_value_formula(
            exponent_bit_length=EXPONENT_BIT_LENGTH, mantissa_bit_length=MANTISSA_BIT_LENGTH
        )
        decoding_formula = get_decode_float_value_formula(
            exponent_bit_length=EXPONENT_BIT_LENGTH, mantissa_bit_length=MANTISSA_BIT_LENGTH
        )
        ...
        while f"C{constant_index}" in physical_value:
            data_records.append(
                CustomFormulaDataRecord(
                    name=f"C{constant_index}#{scaling_byte_number}",
                    length=length,
...
def get_coefficients_formula(scaling_byte_number: int) -> Callable[[int], tuple[CustomFormulaDataRecord, ...]]:
    ...
    :return: Formula for given order number.
    """
    return lambda formula_identifier: get_coefficients(
        formula_identifier=formula_identifier, scaling_byte_number=scaling_byte_number
    )
...
def get_security_access_request(subfunction: int) -> tuple[RawDataRecord]:
    ...
    :param subfunction: SecurityAccess SubFunction value.
...
def get_file_sizes(file_size_parameter_length: int) -> tuple[RawDataRecord, RawDataRecord]:
    ...
    return (
        RawDataRecord(name="fileSizeUnCompressed", length=8 * file_size_parameter_length, unit="bytes"),
        RawDataRecord(name="fileSizeCompressed", length=8 * file_size_parameter_length, unit="bytes"),
    )
...
def get_file_sizes_or_dir_info(file_size_or_dir_info_parameter_length: int) -> tuple[RawDataRecord, RawDataRecord]:
    ...
    return (
        ...
        RawDataRecord(name="fileSizeCompressed", length=8 * file_size_or_dir_info_parameter_length, unit="bytes"),
    )
...
def get_data(memory_size_length: int) -> tuple[RawDataRecord]:
    ...
    return (
        RawDataRecord(name="data", length=8, min_occurrences=memory_size_length, max_occurrences=memory_size_length),
    )
...
def get_secured_data_transmission_request(
    ...
    if signature_length == 0:
        return (ANTI_REPLAY_COUNTER, INTERNAL_SID, INTERNAL_REQUEST_PARAMETERS)
    signature = RawDataRecord(
        name="Signature/MAC",
        length=8,
    ...
    return (ANTI_REPLAY_COUNTER, INTERNAL_SID, INTERNAL_REQUEST_PARAMETERS, signature)


def get_secured_data_transmission_response(
    signature_length: int,
) -> (
    tuple[RawDataRecord, RawDataRecord, RawDataRecord, RawDataRecord]
    | tuple[RawDataRecord, RawDataRecord, RawDataRecord]
    ...
    """
    if signature_length == 0:
        return (ANTI_REPLAY_COUNTER, INTERNAL_RSID, INTERNAL_RESPONSE_PARAMETERS)
    signature = RawDataRecord(
        name="Signature/MAC",
        length=8,
        min_occurrences=signature_length,
    ...
    return (ANTI_REPLAY_COUNTER, INTERNAL_RSID, INTERNAL_RESPONSE_PARAMETERS, signature)
...
def get_event_window_time_2020(event_number: int | None = None) -> MappingDataRecord:
    ...
    """
    return MappingDataRecord(
        name="eventWindowTime" if event_number is None else f"eventWindowTime#{event_number}",
        length=8,
...
def get_event_type_record_01(event_number: int | None = None) -> RawDataRecord:
    ...
    """
    return RawDataRecord(
        name="eventTypeRecord" if event_number is None else f"eventTypeRecord#{event_number}",
        length=8,
...
def get_event_type_record_08_2020(event_number: int | None = None) -> RawDataRecord:
    ...
    return RawDataRecord(
        name="eventTypeRecord" if event_number is None else f"eventTypeRecord#{event_number}",
        length=8,
        children=(RESERVED_BIT, REPORT_TYPE_2020),
    )
...
def get_event_type_record_09_2020_continuation(event_number: int | None = None) -> ConditionalMappingDataRecord:
    ...
    if event_number is None:
        return ConditionalMappingDataRecord(
            mapping={
    ...
    return ConditionalMappingDataRecord(
        mapping={
            0x04: (
                MappingDataRecord(
                    name=f"DTCSnapshotRecordNumber#{event_number}",
            ...
            0x06: (
                MappingDataRecord(
                    name=f"DTCExtDataRecordNumber#{event_number}",
            ...
            0x18: (
                ...
                RawDataRecord(name=f"MemorySelection#{event_number}", length=8),
            ),
            0x19: (
                MappingDataRecord(
                    name=f"DTCExtDataRecordNumber#{event_number}",
...
def get_service_to_respond(event_number: int | None = None) -> RawDataRecord:
    ...
    """
    return RawDataRecord(
        name="serviceToRespondToRecord" if event_number is None else f"serviceToRespondToRecord#{event_number}",
        length=8,
        min_occurrences=1,
        max_occurrences=None,
    )
...
def get_activated_events_2020(
    ...
    for event_number in range(1, number_of_activated_events + 1):
        ...
        data_records.append(
            ConditionalMappingDataRecord(
                mapping={
                    0x01: (event_window, get_event_type_record_01(event_number), service_to_respond),
                    0x03: (event_window, get_event_type_record_03_2020(event_number), service_to_respond),
                    0x07: (event_window, get_event_type_record_07_2020(event_number), service_to_respond),
                    0x08: (event_window, get_event_type_record_08_2020(event_number)),
                    0x09: (
                        event_window,
    ...
    for event_number in range(1, number_of_activated_events + 1):
        ...
        data_records.append(
            ConditionalMappingDataRecord(
                mapping={
                    0x01: (event_window, get_event_type_record_01(event_number), service_to_respond),
                    0x02: (event_window, get_event_type_record_02_2013(event_number), service_to_respond),
                    0x03: (event_window, get_event_type_record_03_2013(event_number), service_to_respond),
                    0x07: (event_window, get_event_type_record_07_2013(event_number), service_to_respond),
                },
                value_mask=0x3F,
            )
        )
