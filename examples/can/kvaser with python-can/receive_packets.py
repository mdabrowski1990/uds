...
def main():
    Timer(interval=1, function=kvaser_bus.send, args=(frame_1, )).run()
    record_1 = can_ti.receive_packet()
    pprint(record_1.__dict__)

    Timer(interval=1, function=kvaser_bus.send, args=(frame_2, )).run()
    Timer(interval=2, function=kvaser_bus.send, args=(frame_3, )).run()
    record_2 = can_ti.receive_packet()
