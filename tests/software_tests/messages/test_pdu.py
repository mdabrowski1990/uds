...
class TestAbstractNPDURecord:
    ...
    def test_frame__set__second_attempt(self, old_value, new_value):
        ...
        with pytest.raises(ReassignmentError):
            AbstractNPDURecord.frame.fset(self=self.mock_pdu_record, value=new_value)
        assert self.mock_pdu_record._AbstractNPDURecord__frame == old_value
        self.mock_pdu_record._AbstractNPDURecord__validate_frame.assert_not_called()
    ...
    def test_direction__set__second_attempt(self, old_value, new_value):
        ...
        with pytest.raises(ReassignmentError):
            AbstractNPDURecord.direction.fset(self=self.mock_pdu_record, value=new_value)
        assert self.mock_pdu_record._AbstractNPDURecord__direction == old_value
