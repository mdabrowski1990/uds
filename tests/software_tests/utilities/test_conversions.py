...
class TestFunctions:
    ...
    @pytest.mark.parametrize(
        "bytes_list, endianness, expected_output",
        [
    ...
    @pytest.mark.parametrize("int_value", [None, 5.0, "not an integer value"])
    ...
    @pytest.mark.parametrize("size", [5.0, "not an integer value"])
    ...
    @pytest.mark.parametrize(
        "expected_output, endianness, int_value",
        [
    ...
    @pytest.mark.parametrize(
        "expected_output, endianness, int_value, size",
        [
    ...
    def test_get_decode_signed_value_formula(self, bit_length, encoding_mapping):
        formula = get_signed_value_decoding_formula(bit_length)
        assert all(formula(unsigned_value) == signed_value for unsigned_value, signed_value in encoding_mapping.items())
    ...
    def test_get_encode_signed_value_formula(self, bit_length, encoding_mapping):
        formula = get_signed_value_encoding_formula(bit_length)
        assert all(formula(signed_value) == unsigned_value for unsigned_value, signed_value in encoding_mapping.items())
...
class TestTimeSync:
    ...
    @pytest.mark.parametrize(
        "initial, samples_number, sync_expiration",
        [
            (True, 1, 10),
            (False, 15, 3.69),
        ],
    ...
    def test_init__update(self, mock_hasattr, initial, samples_number, sync_expiration):
        ...
        assert (
            TimeSync.__init__(self.mock_time_sync, samples_number=samples_number, sync_expiration=sync_expiration)
            is None
        )
    ...
    def test_last_sync_timestamp__get(self):
        ...
        assert (
            TimeSync.last_sync_timestamp.fget(self.mock_time_sync) == self.mock_time_sync._TimeSync__last_sync_timestamp
        )
    ...
    def test_is_sync_outdated__get__true(self):
        ...
        self.mock_perf_counter.return_value = MagicMock(__gt__=mock_is_outdated, __sub__=mock_sub)
        assert TimeSync.is_sync_outdated.fget(self.mock_time_sync) is True
    ...
    @pytest.mark.parametrize(
        "is_sync_outdated, time_value, offset",
        [
    ...
    @pytest.mark.parametrize(
        "is_sync_outdated, time_value, min_value, offset",
        [
    ...
    def test_time_to_perf_counter__min_value(self, is_sync_outdated, time_value, min_value, offset):
        ...
        self.mock_time_sync.offset = offset
        assert (
            TimeSync.time_to_perf_counter(self.mock_time_sync, time_value=time_value, min_value=min_value) == min_value
        )
        self.mock_time.sync.call_count == int(is_sync_outdated)

    @pytest.mark.parametrize(
        "is_sync_outdated, time_value, max_value, offset",
        [
    ...
    def test_time_to_perf_counter__max_value(self, is_sync_outdated, time_value, max_value, offset):
        ...
        self.mock_time_sync.offset = offset
        assert (
            TimeSync.time_to_perf_counter(self.mock_time_sync, time_value=time_value, max_value=max_value) == max_value
        )
        self.mock_time.sync.call_count == int(is_sync_outdated)
    ...
    @pytest.mark.parametrize(
        "is_sync_outdated, perf_counter_value, offset",
        [
            (True, 123.456, 789.012),
            (False, 81, -5),
        ],
    ...
    def test_perf_counter_to_time(self, is_sync_outdated, perf_counter_value, offset):
        ...
        self.mock_time_sync.offset = offset
        assert (
            TimeSync.perf_counter_to_time(self.mock_time_sync, perf_counter_value=perf_counter_value)
            == perf_counter_value + offset
        )
        self.mock_time.sync.call_count == int(is_sync_outdated)

    @pytest.mark.parametrize(
        "is_sync_outdated, perf_counter_value, min_value, offset",
        [
    ...
    def test_perf_counter_to_time__min_value(self, is_sync_outdated, perf_counter_value, min_value, offset):
        ...
        self.mock_time_sync.offset = offset
        assert (
            TimeSync.perf_counter_to_time(
                self.mock_time_sync, perf_counter_value=perf_counter_value, min_value=min_value
            )
            == min_value
        )
        self.mock_time.sync.call_count == int(is_sync_outdated)
    ...
    @pytest.mark.parametrize(
        "is_sync_outdated, perf_counter_value, max_value, offset",
        [
    ...
    def test_perf_counter_to_time__max_value(self, is_sync_outdated, perf_counter_value, max_value, offset):
        ...
        assert (
            TimeSync.perf_counter_to_time(
                self.mock_time_sync, perf_counter_value=perf_counter_value, max_value=max_value
            )
            == max_value
        )
        self.mock_time.sync.call_count == int(is_sync_outdated)
