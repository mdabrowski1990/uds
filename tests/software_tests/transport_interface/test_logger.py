...
class TestTransportLogger:
    ...
    def setup_method(self):
        self.mock_transport_logger = Mock(
            spec=TransportLogger, DECORATED_CLASS_NAME_SUFFIX=TransportLogger.DECORATED_CLASS_NAME_SUFFIX
        )
        # patching
    ...
    def test_init__all(
        ...
        assert (
            TransportLogger.__init__(
                self.mock_transport_logger,
                ...
                packet_log_format=packet_log_format,
            )
            is None
        )
        assert self.mock_transport_logger._TransportLogger__logger == self.mock_get_logger.return_value
    ...
    def test_call__instance(self):
        ...
        assert (
            TransportLogger.__call__(self.mock_transport_logger, mock_transport_interface_instance)
    ...
    def test_call__class(self):
        ...
        assert (
            TransportLogger.__call__(self.mock_transport_logger, MockTransportInterfaceClass)
    ...
    def test_logger__get(self):
        ...
        assert (
            TransportLogger.logger.fget(self.mock_transport_logger)
    ...
    def test_message_logging_level__get(self):
        ...
        assert (
            TransportLogger.message_logging_level.fget(self.mock_transport_logger)
    ...
    def test_packet_logging_level__get(self):
        ...
        assert (
            TransportLogger.packet_logging_level.fget(self.mock_transport_logger)
    ...
    def test_log_sending__get(self):
        ...
        assert (
            TransportLogger.log_sending.fget(self.mock_transport_logger)
    ...
    def test_log_receiving__get(self):
        ...
        assert (
            TransportLogger.log_receiving.fget(self.mock_transport_logger)
    ...
    def test_message_log_format__get(self):
        ...
        assert (
            TransportLogger.message_log_format.fget(self.mock_transport_logger)
    ...
    def test_packet_log_format__get(self):
        ...
        assert (
            TransportLogger.packet_log_format.fget(self.mock_transport_logger)
    ...
    @pytest.mark.parametrize(
        "name, log_sending, log_receiving, message_logging_level, packet_logging_level",
        [
    ...
    def test_decorate_class(
        self, mock_type, name, log_sending, log_receiving, message_logging_level, packet_logging_level
    ):
        ...
        if log_receiving and message_logging_level is not None:
            ...
            decorated_methods["async_receive_message"] = (
                self.mock_transport_logger._decorate_message_method.return_value
            )
        ...
        mock_type.assert_called_once_with(
            name + self.mock_transport_logger.DECORATED_CLASS_NAME_SUFFIX, (mock_class,), decorated_methods
        )
    ...
    def test_decorate_instance(
        self, mock_setattr, log_sending, log_receiving, message_logging_level, packet_logging_level
    ):
        mock_class = Mock()
        ...
        assert (
            TransportLogger._decorate_instance(self.mock_transport_logger, mock_instance) == self.mock_copy.return_value
        )
        mock_setattr.assert_has_calls(calls, any_order=True)
    ...
    def test_decorate_message_method(self, mock_wraps, method):
        assert (
            TransportLogger._decorate_message_method(self.mock_transport_logger, method)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                AsyncMock(spec=AbstractTransportInterface).async_send_message,
                (None, "some arg"),
                {"a": 1, "kwarg_arg": Mock()},
            ),
            (
                AsyncMock(spec=AbstractTransportInterface).async_receive_message,
                (),
    ...
    def test_decorate_packet_method(self, mock_wraps, method):
        assert (
            TransportLogger._decorate_packet_method(self.mock_transport_logger, method)
            == mock_wraps.return_value.return_value
        )
        mock_wraps.assert_called_once_with(method)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                AsyncMock(spec=AbstractTransportInterface).async_send_packet,
                (None, "some arg"),
                {"a": 1, "kwarg_arg": Mock()},
            ),
            (
                AsyncMock(spec=AbstractTransportInterface).async_receive_packet,
                (),
                {"param_1": "value_1", "param_2": "value_2"},
            ),
        ],
    ...
    def test_log_message__log(self, level, record):
        ...
        self.mock_transport_logger.logger.log.assert_called_once_with(
            level=level, msg=self.mock_transport_logger.message_log_format.format.return_value
        )
    ...
    def test_log_packet__log(self, level, record):
        ...
        self.mock_transport_logger.logger.log.assert_called_once_with(
            level=level, msg=self.mock_transport_logger.packet_log_format.format.return_value
        )
