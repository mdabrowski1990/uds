...
class TestAbstractSegmenter:
    ...
    def test_init(self, addressing_information):
        assert (
            AbstractSegmenter.__init__(self.mock_abstract_segmenter, addressing_information=addressing_information)
            is None
        )
    ...
    def test_addressing_information__get(self):
        ...
        assert (
            AbstractSegmenter.addressing_information.fget(self.mock_abstract_segmenter)
            == self.mock_abstract_segmenter._AbstractSegmenter__addressing_information
        )

    @pytest.mark.parametrize("addressing_information", [Mock(), "some value"])
    @patch(f"{SCRIPT_LOCATION}.isinstance")
    def test_addressing_information__set(self, mock_isinstance, addressing_information):
        mock_isinstance.return_value = True
        assert (
            AbstractSegmenter.addressing_information.fset(self.mock_abstract_segmenter, addressing_information) is None
        )
        ...
        mock_isinstance.assert_called_once_with(
            addressing_information, self.mock_abstract_segmenter.supported_addressing_information_class
        )

    @pytest.mark.parametrize("addressing_information", [Mock(), "some value"])
    @patch(f"{SCRIPT_LOCATION}.isinstance")
    def test_addressing_information__set__type_error(self, mock_isinstance, addressing_information):
    ...
    def test_is_supported_packet_type(self, mock_isinstance, value, result):
        ...
        mock_isinstance.assert_called_once_with(
            value,
            (
                self.mock_abstract_segmenter.supported_packet_class,
    ...
    def test_is_supported_packets_sequence_type__false__not_sequence(self, mock_isinstance, value):
        mock_isinstance.return_value = False
        assert (
            AbstractSegmenter.is_supported_packets_sequence_type(self=self.mock_abstract_segmenter, packets=value)
            is False
        )
    ...
    def test_is_supported_packets_sequence_type__false__empty_sequence(self):
        self.mock_abstract_segmenter.is_supported_packet_type.return_value = True
        assert (
            AbstractSegmenter.is_supported_packets_sequence_type(self=self.mock_abstract_segmenter, packets=[]) is False
        )
