...
from can import Bus
from can.interfaces.kvaser import KvaserBus  # TODO: causes `Windows fatal exception: access violation`
from can.interfaces.serial import SerialBus  # TODO: causes `Windows fatal exception: access violation`
from can.interfaces.vector import VectorBus  # TODO: causes `Windows fatal exception: access violation`
from can.interfaces.virtual import VirtualBus  # TODO: causes `Windows fatal exception: access violation`
...
class TestPythonCanTransportInterface:
    ...
    @pytest.mark.parametrize(
        "network_manager, addressing_information, configuration_params",
        [
            (Mock(), Mock(), {}),
            (Mock(), Mock(spec=CanAddressingInformation), {"param1": Mock(), "param2": Mock(), "dlc": 8}),
        ],
    )
    def test_init__mandatory_args(self, network_manager, addressing_information, configuration_params):
        assert (
            PythonCanTransportInterface.__init__(
        ...
        self.mock_abstract_can_ti_init.assert_called_once_with(
            network_manager=network_manager, addressing_information=addressing_information, **configuration_params
        )
        self.mock_buffered_reader.assert_has_calls([call(), call(), call()])
        self.mock_async_buffered_reader.assert_has_calls([call(), call(), call()])

    @pytest.mark.parametrize(
        "network_manager, addressing_information, notifier, async_notifier, configuration_params",
        [
            (Mock(), Mock(), Mock(), Mock(), {}),
            (
                Mock(),
    ...
    def test_init__all_args(
        self, network_manager, addressing_information, notifier, async_notifier, configuration_params
    ):
        assert (
            PythonCanTransportInterface.__init__(
                self=self.mock_can_transport_interface,
    ...
    def test_del__valid(self, mock_parent_del):
    ...
    @patch(f"uds.transport_interface.abstract_transport_interface.AbstractTransportInterface.__del__")
    def test_del__attribute_error(self, mock_parent_del):
        mock_can_ti = Mock(spec=PythonCanTransportInterface)
        assert not hasattr(mock_can_ti, "_PythonCanTransportInterface__rx_frames_buffer")
        assert not hasattr(mock_can_ti, "_PythonCanTransportInterface__tx_frames_buffer")
        assert not hasattr(mock_can_ti, "_PythonCanTransportInterface__fc_frames_buffer")
        assert not hasattr(mock_can_ti, "_PythonCanTransportInterface__async_rx_frames_buffer")
        assert not hasattr(mock_can_ti, "_PythonCanTransportInterface__async_tx_frames_buffer")
        assert not hasattr(mock_can_ti, "_PythonCanTransportInterface__async_fc_frames_buffer")
        assert PythonCanTransportInterface.__del__(mock_can_ti) is None
        mock_parent_del.assert_called_once_with()
    ...
    def test_network_manager__get(self, mock_network_manager):
        assert (
            PythonCanTransportInterface.network_manager.fget(self.mock_can_transport_interface) == mock_network_manager
        )
    ...
    def test_backed__get__valid(self, cls, name):
        self.mock_can_transport_interface.network_manager = MagicMock(
            spec=BusABC, __class__=MagicMock(__name__=cls.__name__, __module__=cls.__module__)
        )
    ...
    def test_notifier__get(self):
        ...
        assert (
            PythonCanTransportInterface.notifier.fget(self.mock_can_transport_interface)
    ...
    @pytest.mark.parametrize(
        "value",
        [
            Mock(spec=Notifier, timeout=PythonCanTransportInterface._MIN_NOTIFIER_TIMEOUT + 0.01),
            Mock(spec=Notifier, timeout=2 * PythonCanTransportInterface._MIN_NOTIFIER_TIMEOUT),
        ],
    )
    @patch(f"{SCRIPT_LOCATION}.isinstance")
    def test_notifier__set__valid__with_warning(self, mock_isinstance, value):
        ...
        assert (
            self.mock_can_transport_interface._PythonCanTransportInterface__notifier.timeout
    ...
    @pytest.mark.parametrize(
        "value",
        [
            Mock(spec=Notifier, timeout=PythonCanTransportInterface._MIN_NOTIFIER_TIMEOUT),
            Mock(spec=Notifier, timeout=PythonCanTransportInterface._MIN_NOTIFIER_TIMEOUT - 0.01),
        ],
    )
    @patch(f"{SCRIPT_LOCATION}.isinstance")
    ...
    def test_async_notifier__get(self):
        ...
        assert (
            PythonCanTransportInterface.async_notifier.fget(self.mock_can_transport_interface)
    ...
    def test_async_notifier__set__valid__with_warning(self, mock_isinstance, value):
        ...
        assert (
            self.mock_can_transport_interface._PythonCanTransportInterface__async_notifier.timeout
            == PythonCanTransportInterface._MIN_NOTIFIER_TIMEOUT
        )
        mock_isinstance.assert_called_once_with(value, self.mock_notifier)
        self.mock_warn.assert_called_once()
    ...
    @pytest.mark.parametrize(
        "notifier, rx_buffer, tx_buffer, fc_buffer",
        [
    ...
    def test_is_async_active__get__true(self):
        mock_rx_buffer = Mock(_is_stopped=False)
        mock_tx_buffer = Mock(_is_stopped=False)
        mock_fc_buffer = Mock(_is_stopped=False)
        mock_notifier_configured = Mock(return_value=True)
        mock_notifier = Mock(
            stopped=False,
            find_instances=Mock(return_value=MagicMock(__contains__=mock_notifier_configured)),
    ...
    @pytest.mark.parametrize(
        "notifier, rx_buffer, tx_buffer, fc_buffer",
        [
            (Mock(stopped=True), Mock(_is_stopped=False), Mock(_is_stopped=False), Mock(_is_stopped=False)),
            (Mock(stopped=False), Mock(_is_stopped=True), Mock(_is_stopped=False), Mock(_is_stopped=False)),
            (Mock(stopped=False), Mock(_is_stopped=False), Mock(_is_stopped=True), Mock(_is_stopped=False)),
            (Mock(stopped=False), Mock(_is_stopped=False), Mock(_is_stopped=False), Mock(_is_stopped=True)),
        ],
    )
    ...
    def test_setup_async__no_notifier(self):
        ...
        assert PythonCanTransportInterface.setup_async(self.mock_can_transport_interface, loop=mock_loop) is None
    ...
    def test_setup_async__another_loop_in_notifier(self):
        ...
        self.mock_can_transport_interface.async_notifier = old_notifier
        assert PythonCanTransportInterface.setup_async(self.mock_can_transport_interface, loop=mock_loop) is None
        assert self.mock_can_transport_interface.async_notifier == self.mock_notifier.return_value
        self.mock_notifier.assert_called_once_with(
            bus=self.mock_can_transport_interface.network_manager,
    ...
    def test_teardown_sync__notifier_with_suppressed_warning(self, mock_teardown_sync):
        ...
        self.mock_can_transport_interface.notifier = mock_notifier
        assert (
            PythonCanTransportInterface.teardown_sync(self.mock_can_transport_interface, suppress_warning=True) is None
        )
        assert self.mock_can_transport_interface.notifier is None
    ...
    def test_teardown_async__no_notifier(self, mock_teardown_async):
        self.mock_can_transport_interface.async_notifier = None
        assert PythonCanTransportInterface.teardown_async(self.mock_can_transport_interface) is None
        assert self.mock_can_transport_interface.async_notifier is None
    ...
    def test_teardown_async__notifier_with_suppressed_warning(self, mock_teardown_async):
        ...
        self.mock_can_transport_interface.async_notifier = mock_notifier
        assert (
            PythonCanTransportInterface.teardown_async(self.mock_can_transport_interface, suppress_warning=True) is None
        )
        assert self.mock_can_transport_interface.async_notifier is None
    ...
    def test_wait_for_flow_control(self, packet_records):
        ...
        assert (
            PythonCanTransportInterface._wait_for_flow_control(
                self.mock_can_transport_interface, timeout_timestamp=MagicMock()
            )
            == packet_records[-1]
        )
    ...
    async def test_async_wait_for_flow_control(self, packet_records):
        ...
        assert (
            await PythonCanTransportInterface._async_wait_for_flow_control(
                self.mock_can_transport_interface, timeout_timestamp=MagicMock()
            )
            == packet_records[-1]
        )
    ...
    def test_wait_for_rx_packet__timeout_error(self, timeout):
        ...
        with pytest.raises(TimeoutError):
            PythonCanTransportInterface._wait_for_rx_packet(
                self.mock_can_transport_interface, buffer=mock_frames_buffer, timeout=timeout
            )
    ...
    def test_wait_for_rx_packet__wall_time_timestamp(self, timeout):
        ...
        self.mock_can_transport_interface._INTERFACES_USING_WALL_TIME_TIMESTAMPS = [
            self.mock_can_transport_interface.backend
        ]
        assert (
            PythonCanTransportInterface._wait_for_rx_packet(
                self.mock_can_transport_interface, buffer=mock_frames_buffer, timeout=timeout
        ...
        self.mock_can_transport_interface.addressing_information.is_input_packet.assert_called_once_with(
            can_id=mock_get_message.return_value.arbitration_id, raw_frame_data=mock_get_message.return_value.data
        )
    ...
    async def test_async_wait_for_rx_packet__timeout_error(self, timeout):
        ...
        with pytest.raises(TimeoutError):
            await PythonCanTransportInterface._async_wait_for_rx_packet(
    ...
    async def test_async_wait_for_rx_packet__wall_time_timestamp(self, timeout):
        ...
        assert (
            await PythonCanTransportInterface._async_wait_for_rx_packet(
                self.mock_can_transport_interface, buffer=mock_buffer, timeout=timeout
            )
            == self.mock_can_packet_record.return_value
        )
        ...
        self.mock_can_transport_interface.addressing_information.is_input_packet.assert_called_once_with(
            can_id=mock_frame.arbitration_id, raw_frame_data=mock_frame.data
        )
        self.mock_can_packet_record.assert_called_once_with(
            frame=mock_frame,
            direction=TransmissionDirection.RECEIVED,
    ...
    def test_wait_for_tx_frame__timeout(self):
        ...
        with pytest.raises(TimeoutError):
            PythonCanTransportInterface._wait_for_tx_frame(
                self.mock_can_transport_interface, buffer=Mock(), frame=Mock(), timestamp=mock_timestamp
            )
        mock_is_timeout_reached.assert_called_once()

    @pytest.mark.parametrize(
        "can_frame, observed_can_frame",
        [
            (Mock(arbitration_id=0x123, data=[0x00, 0xFF]), Mock(arbitration_id=0x123, data=[0x00, 0xFF], is_rx=True)),
            (
                Mock(arbitration_id=1, data=(0x52, 0xFF, 0xC0)),
                Mock(arbitration_id=2, data=(0x52, 0xFF, 0xC0), is_rx=False),
            ),
            (Mock(arbitration_id=0x7DF, data=[1, 2, 3, 4]), Mock(arbitration_id=0x7DF, data=[5, 6, 7, 8], is_rx=False)),
        ],
    )
    def test_wait_for_tx_frame__other_then_timeout(self, can_frame, observed_can_frame):
        ...
        with pytest.raises(TimeoutError):
            PythonCanTransportInterface._wait_for_tx_frame(
                self.mock_can_transport_interface, buffer=mock_buffer, frame=can_frame, timestamp=mock_timestamp
            )
        assert mock_is_timeout_reached.call_count == 2
    ...
    @pytest.mark.parametrize(
        "can_frame, observed_can_frame",
        [
            (Mock(arbitration_id=0x123, data=[0x00, 0xFF]), Mock(arbitration_id=0x123, data=[0x00, 0xFF], is_rx=False)),
            (
    ...
    @pytest.mark.parametrize(
        "async_queue_size, sync_queue_size",
        [
            (0, 0),
            (1, 25),
            (69, 3),
    ...
    def test_clear_received_frame_buffers(self, sync_queue_size, async_queue_size):
        ...
        mock_sync_queue = Mock(empty=Mock(side_effect=[False] * sync_queue_size + [True]), get_nowait=sync_get_nowait)
        mock_async_queue = Mock(
            empty=Mock(side_effect=[False] * async_queue_size + [True]), get_nowait=async_get_nowait
        )
    ...
    @pytest.mark.parametrize("bus_cls", [VirtualBus, SerialBus, KvaserBus, VectorBus])
    @patch(f"{SCRIPT_LOCATION}.isinstance")
    def test_is_supported_network_manager__true(self, mock_isinstance, bus_cls):
        mock_isinstance.return_value = True
        value = MagicMock(__class__=MagicMock(__name__=bus_cls.__name__, __module__=bus_cls.__module__))
    ...
    def test_send_packet__observed(self, packet):
        if packet.packet_type == CanPacketType.FLOW_CONTROL:
            timeout = self.mock_can_transport_interface.n_ar_timeout / 1000.0
        else:
            timeout = self.mock_can_transport_interface.n_as_timeout / 1000.0
        ...
        self.mock_can_transport_interface.network_manager.send.assert_called_once_with(
            msg=self.mock_python_can_frame.return_value, timeout=timeout
        )
    ...
    def test_send_packet__not_observed(self, packet):
        ...
        self.mock_python_can_frame.assert_has_calls(
            [
                ...
                call(
                    arbitration_id=self.mock_python_can_frame.return_value.arbitration_id,
    ...
    async def test_async_send_packet__not_observed(self, packet):
        ...
        assert (
            await PythonCanTransportInterface.async_send_packet(
                self.mock_can_transport_interface, packet, loop=mock_loop
            )
            == self.mock_can_packet_record.return_value
        )
        self.mock_can_transport_interface.clear_transmitted_frame_buffers.assert_called_once_with()
    ...
    def test_receive_packet(self, timeout):
        assert (
            PythonCanTransportInterface.receive_packet(self.mock_can_transport_interface, timeout)
        ...
        self.mock_can_transport_interface._wait_for_rx_packet.assert_called_once_with(
            buffer=self.mock_can_transport_interface._PythonCanTransportInterface__rx_frames_buffer, timeout=timeout
        )
        self.mock_can_transport_interface.setup_sync.assert_called_once_with()
    ...
    async def test_async_receive_packet(self, timeout, loop):
        assert (
            await PythonCanTransportInterface.async_receive_packet(
                self.mock_can_transport_interface, timeout=timeout, loop=loop
            )
...
class TestPythonCanTransportInterfacePerformance:
    ...
    def test_wait_for_rx_packet__timeout(self, performance_tolerance_ms, mean_performance_tolerance_ms, timeout):
        def _get_message(*_, **__):
            sleep(0.005)
            return Mock()

        mock_buffer = Mock(spec=BufferedReader, get_message=_get_message)
        self.mock_can_transport_interface.addressing_information.is_input_packet.return_value = None
        ...
        for _ in range(self.REPETITIONS):
            ...
            assert timeout <= execution_time_ms <= timeout + performance_tolerance_ms

        mean_execution_time_ms = sum(execution_times) / len(execution_times)
        assert timeout <= mean_execution_time_ms <= timeout + mean_performance_tolerance_ms

    # _async_wait_for_rx_packet

    @pytest.mark.parametrize("timeout", [10, 75])
    @pytest.mark.asyncio
    async def test_async_wait_for_rx_packet__timeout(
        self, performance_tolerance_ms, mean_performance_tolerance_ms, timeout
    ):
        async def _get_message(*_, **__):
            await asyncio_sleep(0.005)
            return Mock()

        mock_buffer = Mock(spec=AsyncBufferedReader, get_message=_get_message)
        self.mock_can_transport_interface.addressing_information.is_input_packet.return_value = None
    ...
    def test_receive_cf_packets_block__n_cr_timeout(
        self, mock_is_initial_packet_type, performance_tolerance_ms, mean_performance_tolerance_ms, n_cr_timeout
    ):
        def _get_packet_record(*_, **__):
            sleep(0.005)
            return Mock(spec=CanPacketRecord, packet_type=Mock())
        ...
        for _ in range(self.REPETITIONS):
            timestamp_before = perf_counter()
            with pytest.raises(TimeoutError):
                PythonCanTransportInterface._receive_cf_packets_block(
                    self.mock_can_transport_interface,
            ...
            execution_time_ms = (timestamp_after - timestamp_before) * 1000.0
            execution_times.append(execution_time_ms)
            assert n_cr_timeout <= execution_time_ms <= n_cr_timeout + performance_tolerance_ms

        mean_execution_time_ms = sum(execution_times) / len(execution_times)
        assert n_cr_timeout <= mean_execution_time_ms <= n_cr_timeout + mean_performance_tolerance_ms

    @pytest.mark.parametrize("timeout_end", [10, 75])
    @patch(f"{SCRIPT_LOCATION}.CanPacketType.is_initial_packet_type")
    def test_receive_cf_packets_block__end_timeout(
        self, mock_is_initial_packet_type, performance_tolerance_ms, mean_performance_tolerance_ms, timeout_end
    ):
        current_sn = 0
        ...
        for _ in range(self.REPETITIONS):
            current_sn = 0
            timestamp_end = perf_counter() + timeout_end / 1000.0
            with pytest.raises(TimeoutError):
                PythonCanTransportInterface._receive_cf_packets_block(
                    self.mock_can_transport_interface,
            ...
            timestamp_after = perf_counter()
            diff_times.append(abs(timestamp_after - timestamp_end) * 1000.0)
            assert timestamp_end <= timestamp_after <= timestamp_end + performance_tolerance_ms / 1000.0

        mean_diff_time_ms = sum(diff_times) / len(diff_times)
        assert 0 <= mean_diff_time_ms <= mean_performance_tolerance_ms
    ...
    @patch(f"{SCRIPT_LOCATION}.CanPacketType.is_initial_packet_type")
    @pytest.mark.asyncio
    async def test_async_receive_cf_packets_block__n_cr_timeout(
        self, mock_is_initial_packet_type, performance_tolerance_ms, mean_performance_tolerance_ms, n_cr_timeout
    ):
        ...
        for _ in range(self.REPETITIONS):
            timestamp_before = perf_counter()
            with pytest.raises(TimeoutError):
                await PythonCanTransportInterface._async_receive_cf_packets_block(
                    self.mock_can_transport_interface,
                    sequence_number=Mock(),
                    block_size=float("inf"),
                    remaining_data_length=float("inf"),
    ...
    @patch(f"{SCRIPT_LOCATION}.CanPacketType.is_initial_packet_type")
    @pytest.mark.asyncio
    async def test_async_receive_cf_packets_block__end_timeout(
        self, mock_is_initial_packet_type, performance_tolerance_ms, mean_performance_tolerance_ms, timeout_end
    ):
        current_sn = 0
    ...
    def test_receive_consecutive_frames__end_timeout(
        self, end_timeout, performance_tolerance_ms, mean_performance_tolerance_ms
    ):
        current_sn = 0
        ...
        for _ in range(self.REPETITIONS):
            ...
            with pytest.raises(TimeoutError):
                PythonCanTransportInterface._receive_consecutive_frames(
                    ...
                    timestamp_end=timestamp_before + end_timeout / 1000.0,
                )
            timestamp_after = perf_counter()
            execution_time_ms = (timestamp_after - timestamp_before) * 1000.0
            execution_times.append(execution_time_ms)
            assert end_timeout <= execution_time_ms <= end_timeout + performance_tolerance_ms

        mean_execution_time_ms = sum(execution_times) / len(execution_times)
        assert end_timeout <= mean_execution_time_ms <= end_timeout + mean_performance_tolerance_ms

    # _async_receive_consecutive_frames

    @pytest.mark.parametrize("end_timeout", [10, 75])
    @pytest.mark.asyncio
    async def test_async_receive_consecutive_frames__end_timeout(
        self, end_timeout, performance_tolerance_ms, mean_performance_tolerance_ms
    ):
        current_sn = 0
        ...
        [
            ...
            },
            {
                ...
                "flow_control_parameters_generator": DefaultFlowControlParametersGenerator(
                    block_size=10, st_min=50, wait_count=1, repeat_wait=True
                ),
            },
        ],
    )
    def test_init(self, init_kwargs, example_can_addressing_information):
        py_can_ti = PythonCanTransportInterface(
            **init_kwargs, addressing_information=example_can_addressing_information
        )
        ...
        assert py_can_ti.flow_control_parameters_generator == init_kwargs.get(
            "flow_control_parameters_generator", AbstractCanTransportInterface.DEFAULT_FLOW_CONTROL_PARAMETERS
        )
