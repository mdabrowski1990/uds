...
class TestCanFirstFrame:
    ...
    def setup_method(self):
        ...
        self._patcher_dlc_handler = patch(
    ...
    @pytest.mark.parametrize(
        ...
        [
            (Mock(), [FIRST_FRAME_N_PCI << 4, 0, *range(100, 162)], 0, True),
            (Mock(), [(FIRST_FRAME_N_PCI << 4) + 0xF, 0xFF, *range(6)], 0, True),
    ...
    def test_validate_first_frame_data__value_error(self, mock_is_first_frame, addressing_format, raw_frame_data):
    ...
    @patch(f"{SCRIPT_LOCATION}.is_first_frame")
    def test_validate_first_frame_data__valid(
        self,
        mock_is_first_frame,
        mock_extract_ff_dl,
        ...
        self.mock_validate_raw_bytes.assert_called_once_with(raw_frame_data, allow_empty=False)
        mock_is_first_frame.assert_called_once_with(addressing_format=addressing_format, raw_frame_data=raw_frame_data)
        mock_extract_ff_dl.assert_called_once_with(addressing_format=addressing_format, raw_frame_data=raw_frame_data)
        mock_extract_ff_dl_data_bytes.assert_called_once_with(
    ...
    @pytest.mark.parametrize(
        "addressing_format, payload, dlc, data_length, target_address, address_extension, "
    ...
    def test_create_first_frame_data__valid(
        self,
        mock_encode_ff_dl,
        addressing_format,
        payload,
    ...
    @patch(f"{SCRIPT_LOCATION}.encode_ff_dl")
    def test_create_first_frame_data__inconsistent(
        self,
        mock_encode_ff_dl,
        addressing_format,
        payload,
    ...
    @pytest.mark.parametrize(
        "addressing_format, payload, dlc, ff_dl, long_ff_dl_format, target_address, "
    ...
    def test_generate_first_frame_data__valid(
        self,
        mock_generate_ff_dl_bytes,
        addressing_format,
        payload,
        dlc,
        ff_dl,
    ...
    @patch(f"{SCRIPT_LOCATION}.generate_ff_dl_bytes")
    def test_generate_first_frame_data__inconsistent(
        self,
        mock_generate_ff_dl_bytes,
        addressing_format,
        payload,
    ...
    @pytest.mark.parametrize(
        "addressing_format, raw_frame_data, ai_bytes_number, ff_dl_data_bytes_number",
        [
    ...
    @patch(f"{SCRIPT_LOCATION}.extract_ff_dl_data_bytes")
    def test_extract_first_frame_payload(
        self, mock_extract_ff_dl_data_bytes, addressing_format, raw_frame_data, ai_bytes_number, ff_dl_data_bytes_number
    ):
        ...
        assert (
            extract_first_frame_payload(addressing_format=addressing_format, raw_frame_data=raw_frame_data)
    ...
    @patch(f"{SCRIPT_LOCATION}.extract_ff_dl_data_bytes")
    def test_extract_ff_dl__valid(
        self, mock_extract_ff_dl_data_bytes, addressing_format, raw_frame_data, ff_dl_data_bytes, expected_output
    ):
    ...
    @patch(f"{SCRIPT_LOCATION}.extract_ff_dl_data_bytes")
    def test_extract_ff_dl__not_implemented(
        self, mock_extract_ff_dl_data_bytes, addressing_format, raw_frame_data, ff_dl_data_bytes
    ):
        mock_extract_ff_dl_data_bytes.return_value = ff_dl_data_bytes
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                Mock(),
                [0x10, 0x00, 0x01, 0xFF, 0xAB, 0xCD, 0xFF, 0xFF],
                0,
                bytearray([0x10, 0x00, 0x01, 0xFF, 0xAB, 0xCD]),
            ),
            (
                Mock(),
                [0x00, 0x10, 0x00, 0x01, 0xFF, 0xAB, 0xCD, 0xFF],
                1,
                bytearray([0x10, 0x00, 0x01, 0xFF, 0xAB, 0xCD]),
            ),
        ],
    ...
    def test_extract_ff_dl_data_bytes(self, addressing_format, raw_frame_data, ai_data_bytes_number, expected_output):
        self.mock_can_addressing_information.get_ai_data_bytes_number.return_value = ai_data_bytes_number
        assert (
            extract_ff_dl_data_bytes(addressing_format=addressing_format, raw_frame_data=raw_frame_data)
            == expected_output
        )
        self.mock_can_addressing_information.get_ai_data_bytes_number.assert_called_once_with(addressing_format)
    ...
    @patch(f"{SCRIPT_LOCATION}.generate_ff_dl_bytes")
    @patch(f"{SCRIPT_LOCATION}.validate_ff_dl")
    def test_encode_valid_ff_dl(self, mock_validate_ff_dl, mock_generate_ff_dl_bytes, addressing_format, dlc, ff_dl):
        assert (
            encode_ff_dl(addressing_format=addressing_format, dlc=dlc, ff_dl=ff_dl)
            == mock_generate_ff_dl_bytes.return_value
        )
        mock_validate_ff_dl.assert_called_once_with(addressing_format=addressing_format, dlc=dlc, ff_dl=ff_dl)
    ...
    @pytest.mark.parametrize(
        "addressing_format, dlc, long_ff_dl_format",
        [
    ...
    def test_get_first_frame_payload_size__value_error(self, addressing_format, dlc, long_ff_dl_format):
        with pytest.raises(ValueError):
            get_first_frame_payload_size(
                addressing_format=addressing_format, dlc=dlc, long_ff_dl_format=long_ff_dl_format
            )

    @pytest.mark.parametrize(
        "addressing_format, dlc, long_ff_dl_format, data_bytes_number, ai_data_bytes_number",
        [
    ...
    def test_get_first_frame_payload_size__valid(
        self, addressing_format, dlc, long_ff_dl_format, data_bytes_number, ai_data_bytes_number
    ):
    ...
    @pytest.mark.parametrize(
        "ff_dl, dlc, addressing_format, sf_dl",
        [
    ...
    @pytest.mark.parametrize(
        "ff_dl, dlc, addressing_format, ff_dl_bytes_number",
        [
    ...
    def test_validate_ff_dl__inconsistent_format(self, ff_dl, dlc, addressing_format, ff_dl_bytes_number):
        ...
        with pytest.raises(InconsistencyError):
            validate_ff_dl(
                ff_dl=ff_dl, dlc=dlc, addressing_format=addressing_format, ff_dl_bytes_number=ff_dl_bytes_number
            )
        self.mock_get_max_sf_dl.assert_called_once_with(dlc=dlc, addressing_format=addressing_format)
...
class TestCanFirstFrameIntegration:
    """Integration tests for functions in CAN First Frame module."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (CanAddressingFormat.NORMAL_FIXED_ADDRESSING, [0x10, 0x00, 0x00, 0x00, 0x10, 0x00, *range(58)]),
            (CanAddressingFormat.EXTENDED_ADDRESSING, [0x10, 0x1F, 0xFF, *range(100, 121)]),
            (CanAddressingFormat.MIXED_11BIT_ADDRESSING, (0x0F, 0x10, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF)),
            (CanAddressingFormat.MIXED_29BIT_ADDRESSING, [0x0F, 0x10, 0x3E, *range(50, 111)]),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                bytearray([0x1F, 0xED, 0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC]),
            ),
            (
                {
                ...
                bytearray([0x10, 0x00, 0xFF, 0xFF, 0xFF, 0xFF, 0xF0, 0xE1, 0xD2, 0xC3, 0xB4, 0xA5]),
            ),
            (
                {
                ...
                bytearray([0xC0, 0x10, 0x3E, *range(120, 181)]),
            ),
            (
                {
                ...
                bytearray([0x0B, 0x10, 0x00, 0x00, 0x00, 0x10, 0x00, *range(9)]),
            ),
            (
                {
                ...
                bytearray([0x61, 0x1F, 0xFF, 0x9A, 0x8B, 0x7C, 0x6D, 0x5E]),
            ),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                bytearray([0x1F, 0xED, 0x12, 0x34, 0x56, 0x78, 0x9A]),
            ),
            (
                {
                    "addressing_format": CanAddressingFormat.NORMAL_FIXED_ADDRESSING,
                ...
                bytearray([0x10, 0x00, 0x00, 0x00, 0x0F, 0xED, 0x78, 0x9A]),
            ),
            (
                {
                    "addressing_format": CanAddressingFormat.EXTENDED_ADDRESSING,
                ...
                bytearray([0xC0, 0x10, 0x3D]) + bytearray(range(120, 181)),
            ),
            (
                {
                    "addressing_format": CanAddressingFormat.MIXED_11BIT_ADDRESSING,
                ...
                bytearray([0x0B, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00]) + bytearray(range(100, 109)),
            ),
            (
                {
                    "addressing_format": CanAddressingFormat.MIXED_29BIT_ADDRESSING,
                ...
                bytearray([0x61, 0x10, 0x04, 0x9A, 0x8B, 0x7C, 0x6D, 0x5E]),
            ),
        ],
    )
    ...
    @pytest.mark.parametrize(
        "addressing_format, raw_frame_data, payload_i",
        [
    ...
    def test_extract_first_frame_payload(self, addressing_format, raw_frame_data, payload_i):
        assert (
            extract_first_frame_payload(addressing_format=addressing_format, raw_frame_data=raw_frame_data)
            == bytearray(raw_frame_data)[payload_i:]
        )
