...
class TestSingleFrame:
    ...
    @patch(f"{SCRIPT_LOCATION}.CanDlcHandler.encode_dlc")
    ...
    def test_validate_frame_data__valid(self, mock_is_first_frame, mock_extract_ff_dl, mock_extract_ff_dl_data_bytes,
                                        mock_validate_ff_dl, mock_encode_dlc,
                                        addressing_format, raw_frame_data):
        ...
        mock_extract_ff_dl_data_bytes.assert_called_once_with(addressing_format=addressing_format,
                                                              raw_frame_data=raw_frame_data)
        mock_encode_dlc.assert_called_once_with(len(raw_frame_data))
        mock_validate_ff_dl.assert_called_once_with(addressing_format=addressing_format,
                                                    dlc=mock_encode_dlc.return_value,
    ...
    @patch(f"{SCRIPT_LOCATION}.CanDlcHandler.decode_dlc")
    def test_get_payload_size__valid(self, mock_decode_dlc,
    ...
    @patch(f"{SCRIPT_LOCATION}.validate_ff_dl")
    def test_generate_ff_dl_bytes(self, mock_validate_ff_dl,
        ...
        if long_ff_dl_format:
            mock_validate_ff_dl.assert_called_once_with(ff_dl=ff_dl, ff_dl_bytes_number=LONG_FF_DL_BYTES_USED)
        else:
            mock_validate_ff_dl.assert_called_once_with(ff_dl=ff_dl, ff_dl_bytes_number=SHORT_FF_DL_BYTES_USED)
...
#     @pytest.mark.parametrize("ai_data_bytes, ff_dl_data_bytes, payload", [
#         (bytearray(), bytearray([0x12, 0x34]), range(6)),
#         (bytearray([0x9F]), bytearray([0x10, 0x00, 0xFE, 0xDC, 0xBA, 0x98]), list(range(90, 142))),
#     ])
...
#     ])
#     @pytest.mark.parametrize("ai_data_bytes, ff_dl_data_bytes, payload, expected_frame_length", [
#         (bytearray(), bytearray([0x12, 0x34]), range(6), 7),
#         (bytearray([0x9F]), bytearray([0x10, 0x00, 0xFE, 0xDC, 0xBA, 0x98]), list(range(90, 142)), 64),
#     ])
#     @patch(f"{SCRIPT_LOCATION}.CanFirstFrameHandler._CanFirstFrameHandler__encode_any_ff_dl")
...
#           "long_ff_dl_format": True,
#           "payload": list(range(9)),
#           "address_extension": 0x0B}, bytearray([0x0B, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00]) + bytearray(range(9))),
#         ({"addressing_format": CanAddressingFormat.MIXED_29BIT_ADDRESSING,
#           "dlc": 8,
#           "ff_dl": 0x6,
#           "long_ff_dl_format": False,
#           "payload": [0x9A, 0x8B, 0x7C, 0x6D, 0x5E],
#           "target_address": 0x9E,
#           "address_extension": 0x61}, bytearray([0x61, 0x10, 0x06, 0x9A, 0x8B, 0x7C, 0x6D, 0x5E]))
#     ])
...
#         (CanAddressingFormat.NORMAL_ADDRESSING, (0x10, 0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE), 0x12),
