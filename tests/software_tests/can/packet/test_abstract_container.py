...
class TestAbstractCanPacketContainer:
    ...
    def setup_method(self):
        ...
        mock_can_packet_type_class = Mock(
            SINGLE_FRAME=CanPacketType.SINGLE_FRAME,
    ...
    def test_dlc__get(self, raw_frame_data):
        ...
        assert (
            AbstractCanPacketContainer.dlc.fget(self.mock_can_packet_container)
    ...
    def test_target_address__get(self, ai_info):
        ...
        assert (
            AbstractCanPacketContainer.target_address.fget(self.mock_can_packet_container) == ai_info["target_address"]
        )
    ...
    def test_source_address__get(self, ai_info):
        self.mock_can_ai_class.decode_frame_ai_params.return_value = ai_info
        assert (
            AbstractCanPacketContainer.source_address.fget(self.mock_can_packet_container) == ai_info["source_address"]
        )
        self.mock_can_ai_class.decode_frame_ai_params.assert_called_once_with(
            addressing_format=self.mock_can_packet_container.addressing_format,
            can_id=self.mock_can_packet_container.can_id,
    ...
    def test_address_extension__get(self, ai_info):
        self.mock_can_ai_class.decode_frame_ai_params.return_value = ai_info
        assert (
            AbstractCanPacketContainer.address_extension.fget(self.mock_can_packet_container)
    ...
    @pytest.mark.parametrize(
        "ai_data_bytes_number, raw_frame_data",
        [
            (0, [0x2F]),
    ...
    def test_packet_type__get(self, ai_data_bytes_number, raw_frame_data):
        ...
        self.mock_can_packet_container.raw_frame_data = raw_frame_data
        assert (
            AbstractCanPacketContainer.packet_type.fget(self.mock_can_packet_container)
    ...
    @pytest.mark.parametrize(
        "packet_type", [CanPacketType.SINGLE_FRAME, CanPacketType.FIRST_FRAME, CanPacketType.FLOW_CONTROL]
    )
    ...
    @pytest.mark.parametrize(
        "packet_type", [CanPacketType.SINGLE_FRAME, CanPacketType.FIRST_FRAME, CanPacketType.CONSECUTIVE_FRAME]
    )
