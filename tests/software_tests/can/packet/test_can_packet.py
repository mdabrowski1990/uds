...
class TestCanPacket:
    ...
    @pytest.mark.parametrize(
        "addressing_format, packet_type, addressing_type, can_id, target_address, source_address, "
        ...
        [
            ...
            (
                ...
                0x90,
                8,
                {"ff_dl": 123, "payload": [0x12, 0x34, 0x56, 0x78, 0x89]},
            ),
        ],
    )
    def test_init__all_args(
        ...
        self.mock_can_packet.set_addressing_information.assert_called_once_with(
            addressing_type=addressing_type,
            can_id=can_id,
            target_address=target_address,
            source_address=source_address,
        ...
        self.mock_can_packet.set_packet_data.assert_called_once_with(
            packet_type=packet_type, dlc=dlc, **packet_type_specific_kwargs
        )

    @pytest.mark.parametrize(
        "addressing_format, packet_type, addressing_type, packet_type_specific_kwargs",
        [
            (Mock(), Mock(), Mock(), {}),
            (
    ...
    def test_init__mandatory_args(self, addressing_format, packet_type, addressing_type, packet_type_specific_kwargs):
        ...
        self.mock_can_packet.set_packet_data.assert_called_once_with(
            packet_type=packet_type, dlc=None, **packet_type_specific_kwargs
        )
    ...
    def test_can_id__get(self):
        ...
        assert CanPacket.can_id.fget(self.mock_can_packet) == self.mock_can_packet._CanPacket__can_id

    # raw_frame_data

    def test_raw_frame_data__get(self):
        self.mock_can_packet._CanPacket__raw_frame_data = Mock()
        assert CanPacket.raw_frame_data.fget(self.mock_can_packet) == self.mock_can_packet._CanPacket__raw_frame_data
    ...
    def test_addressing_format__get(self):
        ...
        assert (
            CanPacket.addressing_format.fget(self.mock_can_packet) == self.mock_can_packet._CanPacket__addressing_format
        )
    ...
    def test_addressing_type__get(self):
        ...
        assert CanPacket.addressing_type.fget(self.mock_can_packet) == self.mock_can_packet._CanPacket__addressing_type

    # set_addressing_information

    @pytest.mark.parametrize(
        "addressing_type, ai_data_bytes, raw_frame_data",
        [
            (Mock(), bytearray([0x3B]), b"\xfe\xdc\xba\x98\x76\x54\x32\x10"),
            (AddressingType.FUNCTIONAL, bytearray(), b""),
        ],
    )
    def test_set_addressing_information__mandatory_args(self, addressing_type, ai_data_bytes, raw_frame_data):
        ...
        assert CanPacket.set_addressing_information(self.mock_can_packet, addressing_type=addressing_type) is None
        assert (
        ...
        assert (
            self.mock_can_packet._CanPacket__raw_frame_data[len(ai_data_bytes) :]
        ...
        self.mock_can_addressing_information.encode_ai_data_bytes.assert_called_once_with(
            addressing_format=self.mock_can_packet.addressing_format,
            target_address=self.mock_can_addressing_information.validate_addressing_params.return_value[
                "target_address"
            ],
            address_extension=self.mock_can_addressing_information.validate_addressing_params.return_value[
                "address_extension"
            ],
        )

    @pytest.mark.parametrize(
        "addressing_type, can_id, target_address, source_address, address_extension, ai_data_bytes, raw_frame_data",
        [
    ...
    def test_set_addressing_information__all_args(
        self, addressing_type, can_id, target_address, source_address, address_extension, ai_data_bytes, raw_frame_data
    ):
        self.mock_can_packet._CanPacket__raw_frame_data = raw_frame_data
        self.mock_can_addressing_information.encode_ai_data_bytes.return_value = ai_data_bytes
        assert (
            CanPacket.set_addressing_information(
                self.mock_can_packet,
                addressing_type=addressing_type,
                can_id=can_id,
                target_address=target_address,
    ...
    def test_set_packet_data__single_frame(self, packet_params):
        assert (
            CanPacket.set_packet_data(self.mock_can_packet, packet_type=CanPacketType.SINGLE_FRAME, **packet_params)
            is None
        )
    ...
    def test_set_packet_data__first_frame(self, packet_params):
        assert (
            CanPacket.set_packet_data(self.mock_can_packet, packet_type=CanPacketType.FIRST_FRAME, **packet_params)
            is None
        )
        if "dlc" not in packet_params:
            packet_params["dlc"] = None
    ...
    def test_set_packet_data__consecutive_frame(self, packet_params):
        assert (
            CanPacket.set_packet_data(
                self.mock_can_packet, packet_type=CanPacketType.CONSECUTIVE_FRAME, **packet_params
            )
            is None
        )
        if "dlc" not in packet_params:
    ...
    def test_set_packet_data__flow_control(self, packet_params):
        assert (
            CanPacket.set_packet_data(self.mock_can_packet, packet_type=CanPacketType.FLOW_CONTROL, **packet_params)
            is None
        )
        if "dlc" not in packet_params:
            packet_params["dlc"] = None
    ...
    def test_set_packet_data__not_implemented(self):
        ...
        with pytest.raises(NotImplementedError):
            CanPacket.set_packet_data(self.mock_can_packet, packet_type=mock_packet_type)
...
class TestCanPacketIntegration:
    ...
    @pytest.mark.parametrize(
        "init_kwargs, expected_attribute_values",
        [
            # SF
            (
                {
