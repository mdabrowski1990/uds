...
class TestCanConsecutiveFrame:
    ...
    def setup_method(self):
        ...
        self._patcher_can_dlc_handler = patch(
            f"{SCRIPT_LOCATION}.CanDlcHandler",
            Mock(
                MAX_DATA_BYTES_NUMBER=CanDlcHandler.MAX_DATA_BYTES_NUMBER,
    ...
    def test_is_consecutive_frame(self, addressing_format, raw_frame_data, ai_data_bytes_number, expected_output):
        ...
        assert (
            is_consecutive_frame(addressing_format=addressing_format, raw_frame_data=raw_frame_data) is expected_output
        )
    ...
    def test_validate_consecutive_frame_data__value_error(
        self, mock_is_consecutive_frame, mock_get_consecutive_frame_min_dlc, addressing_format, raw_frame_data
    ):
        mock_is_consecutive_frame.return_value = False
        with pytest.raises(ValueError):
            validate_consecutive_frame_data(addressing_format=addressing_format, raw_frame_data=raw_frame_data)
        mock_is_consecutive_frame.assert_called_once_with(
    ...
    @pytest.mark.parametrize(
        "addressing_format, raw_frame_data, decoded_dlc, min_dlc",
        [
            (Mock(), MagicMock(), 4, 5),
    ...
    @patch(f"{SCRIPT_LOCATION}.get_consecutive_frame_min_dlc")
    @patch(f"{SCRIPT_LOCATION}.is_consecutive_frame")
    def test_validate_consecutive_frame_data__inconsistent(
        self,
        mock_is_consecutive_frame,
    ...
    @patch(f"{SCRIPT_LOCATION}.get_consecutive_frame_min_dlc")
    @patch(f"{SCRIPT_LOCATION}.is_consecutive_frame")
    def test_validate_consecutive_frame_data__valid(
        self,
        mock_is_consecutive_frame,
        mock_get_consecutive_frame_min_dlc,
        addressing_format,
        ...
        self.mock_can_dlc_handler.encode_dlc.return_value = decoded_dlc
        assert (
            validate_consecutive_frame_data(addressing_format=addressing_format, raw_frame_data=raw_frame_data) is None
        )
        mock_is_consecutive_frame.assert_called_once_with(
            addressing_format=addressing_format, raw_frame_data=raw_frame_data
        )
    ...
    @pytest.mark.parametrize(
        "addressing_format, payload, sequence_number, ai_data_bytes, sn_bytes",
        [
    ...
    def test_create_consecutive_frame_data__valid_mandatory_args(
        self,
        mock_encode_sequence_number,
        mock_get_consecutive_frame_min_dlc,
        addressing_format,
        payload,
        sequence_number,
        ...
        assert (
            create_consecutive_frame_data(
                addressing_format=addressing_format, payload=payload, sequence_number=sequence_number
            )
            == expected_output
        )
        mock_get_consecutive_frame_min_dlc.assert_called_once_with(
    ...
    @pytest.mark.parametrize(
        "addressing_format, payload, sequence_number, dlc, filler_byte, target_address, "
    ...
    @patch(f"{SCRIPT_LOCATION}.get_consecutive_frame_min_dlc")
    @patch(f"{SCRIPT_LOCATION}.encode_sequence_number")
    def test_create_consecutive_frame_data__valid_all_args(
        self,
        mock_encode_sequence_number,
        mock_get_consecutive_frame_min_dlc,
        addressing_format,
    ...
    @patch(f"{SCRIPT_LOCATION}.get_consecutive_frame_min_dlc")
    @patch(f"{SCRIPT_LOCATION}.encode_sequence_number")
    def test_create_consecutive_frame_data__inconsistent(
        self,
        mock_encode_sequence_number,
        mock_get_consecutive_frame_min_dlc,
        addressing_format,
    ...
    @pytest.mark.parametrize(
        "addressing_format, payload, sequence_number, dlc, ai_data_bytes, sn_bytes, data_bytes_number",
        [
            (
                Mock(),
    ...
    @patch(f"{SCRIPT_LOCATION}.encode_sequence_number")
    def test_generate_consecutive_frame_data__valid_mandatory_args(
        self,
        mock_encode_sequence_number,
        addressing_format,
        payload,
        ...
        assert (
            generate_consecutive_frame_data(
                addressing_format=addressing_format, payload=payload, sequence_number=sequence_number, dlc=dlc
            )
            == expected_output
        )
        self.mock_can_addressing_information.encode_ai_data_bytes.assert_called_once_with(
    ...
    @patch(f"{SCRIPT_LOCATION}.encode_sequence_number")
    def test_generate_consecutive_frame_data__valid_all_args(
        self,
        mock_encode_sequence_number,
        addressing_format,
        payload,
    ...
    @patch(f"{SCRIPT_LOCATION}.encode_sequence_number")
    def test_generate_consecutive_frame_data__inconsistent(
        self,
        mock_encode_sequence_number,
        addressing_format,
        payload,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                Mock(),
                bytearray([0xFF, 0x20, 0x0F, 0x1E, 0x2D, 0x3C, 0x4B, 0x5A, 0x69, 0x78, 0x87, 0x96, 0xA5, 0xB4]),
                1,
            ),
        ],
    )
    def test_extract_consecutive_frame_payload(self, addressing_format, raw_frame_data, ai_bytes_number):
        self.mock_can_addressing_information.get_ai_data_bytes_number.return_value = ai_bytes_number
        assert (
            extract_consecutive_frame_payload(addressing_format=addressing_format, raw_frame_data=raw_frame_data)
    ...
    @pytest.mark.parametrize(
        "addressing_format, payload_length, ai_data_bytes_number",
        [
            (Mock(), 1, 1),
            (Mock(), 1, 0),
    ...
    def test_get_min_consecutive_frame_dlc__type_error(self, mock_isinstance, addressing_format, payload_length):
    ...
    @pytest.mark.parametrize(
        "addressing_format, dlc, ai_data_bytes_number, data_bytes_number",
        [
            (Mock(), Mock(), 1, 2),
            (Mock(), Mock(), 0, 0),
    ...
    def test_get_consecutive_frame_max_payload_size__inconsistent(
        self, addressing_format, dlc, ai_data_bytes_number, data_bytes_number
    ):
        self.mock_can_dlc_handler.decode_dlc.return_value = data_bytes_number
        self.mock_can_addressing_information.get_ai_data_bytes_number.return_value = ai_data_bytes_number
        with pytest.raises(InconsistencyError):
    ...
    def test_get_consecutive_frame_max_payload_size__valid_with_dlc(
        self, addressing_format, dlc, ai_data_bytes_number, data_bytes_number
    ):
        self.mock_can_dlc_handler.decode_dlc.return_value = data_bytes_number
        self.mock_can_addressing_information.get_ai_data_bytes_number.return_value = ai_data_bytes_number
        assert (
            get_consecutive_frame_max_payload_size(addressing_format=addressing_format, dlc=dlc)
            == data_bytes_number - ai_data_bytes_number - SN_BYTES_USED
        )
        self.mock_can_dlc_handler.decode_dlc.assert_called_once_with(dlc)
        self.mock_can_addressing_information.get_ai_data_bytes_number.assert_called_once_with(addressing_format)
    ...
    def test_get_consecutive_frame_max_payload_size__valid_without_dlc(self, addressing_format, ai_data_bytes_number):
        self.mock_can_addressing_information.get_ai_data_bytes_number.return_value = ai_data_bytes_number
        assert (
            get_consecutive_frame_max_payload_size(addressing_format=addressing_format)
    ...
    @pytest.mark.parametrize(
        "addressing_format, raw_frame_data, ai_bytes_number, expected_sn",
        [
    ...
    def test_extract_sequence_number(self, addressing_format, raw_frame_data, ai_bytes_number, expected_sn):
        self.mock_can_addressing_information.get_ai_data_bytes_number.return_value = ai_bytes_number
        assert (
            extract_sequence_number(addressing_format=addressing_format, raw_frame_data=raw_frame_data) == expected_sn
        )
        self.mock_can_addressing_information.get_ai_data_bytes_number.assert_called_once_with(addressing_format)
...
class TestCanConsecutiveFrameIntegration:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {"addressing_format": CanAddressingFormat.NORMAL_ADDRESSING, "payload": b"\x9a", "sequence_number": 0},
                bytearray([0x20, 0x9A]),
            ),
            (
                {
                ...
                bytearray([0x2F, *range(48)] + (15 * [DEFAULT_FILLER_BYTE])),
            ),
            (
                {
                ...
                bytearray([0xF1, 0x25, 0x92, 0xB8, 0xD9, 0xD9, 0xD9, 0xD9]),
            ),
            (
                {
                ...
                bytearray([0xE8, 0x2B, *range(10, 20)]),
            ),
            (
                {
                ...
                bytearray([0xDC, 0x21, *range(50, 96)]),
            ),
        ],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ...
                bytearray([0x2F, 0xFE, 0xDC, DEFAULT_FILLER_BYTE, DEFAULT_FILLER_BYTE]),
            ),
            (
                {
                    "addressing_format": CanAddressingFormat.EXTENDED_ADDRESSING,
    ...
    @pytest.mark.parametrize(
        "addressing_format, raw_frame_data, payload",
        [
            (
                CanAddressingFormat.NORMAL_ADDRESSING,
                b"\x2f\x12\x34\x56\x78\x9a\xbc\xde",
