...
class TestCanIdHandler:
    ...
    def test_is_can_id(self, mock_is_standard_id, mock_is_extended_can_id, value, is_standard_id, is_extended_id):
    ...
    @pytest.mark.parametrize(
        "value",
        [
            CanIdHandler.MIN_STANDARD_VALUE,
    ...
    @pytest.mark.parametrize(
        "value",
        [
            CanIdHandler.MIN_STANDARD_VALUE - 1,
    ...
    @pytest.mark.parametrize(
        "value",
        [
            CanIdHandler.MIN_EXTENDED_VALUE,
    ...
    @pytest.mark.parametrize(
        "value",
        [
            CanIdHandler.MIN_EXTENDED_VALUE - 1,
    ...
    @pytest.mark.parametrize("value", [None, 5.0, "not a CAN ID"])
    ...
    def test_validate_can_id__value_error(
        self, mock_is_can_id, mock_is_standard_can_id, mock_is_extended_can_id, value, extended_can_id
    ):
    ...
    @patch(f"{SCRIPT_LOCATION}.CanIdHandler.is_standard_can_id")
    @patch(f"{SCRIPT_LOCATION}.CanIdHandler.is_can_id")
    def test_validate_can_id__valid(
        self, mock_is_can_id, mock_is_standard_can_id, mock_is_extended_can_id, value, extended_can_id
    ):
    ...
    @pytest.mark.parametrize("value", [CanIdHandler.MIN_PRIORITY_VALUE - 1, CanIdHandler.MAX_PRIORITY_VALUE + 1])
    @patch(f"{SCRIPT_LOCATION}.isinstance")
    ...
    @pytest.mark.parametrize(
        "value",
        [CanIdHandler.MIN_PRIORITY_VALUE, CanIdHandler.DEFAULT_PRIORITY_VALUE - 1, CanIdHandler.MAX_PRIORITY_VALUE],
    )
    @patch(f"{SCRIPT_LOCATION}.isinstance")
...
class TestCanDlcHandler:
    ...
    @pytest.mark.parametrize("value", [None, 2.0, "not a number of bytes"])
...
class TestCanDlcHandlerIntegration:
    ...
    @pytest.mark.parametrize(
        "data_bytes_number", [CanDlcHandler.MIN_DATA_BYTES_NUMBER, 8, CanDlcHandler.MAX_DATA_BYTES_NUMBER]
    )
