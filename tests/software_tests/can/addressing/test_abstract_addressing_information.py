...
class TestAbstractCanAddressingInformation:
    ...
    def test_init(
        self, mock_abstract_ai_init, rx_physical_params, tx_physical_params, rx_functional_params, tx_functional_params
    ):
        assert (
            AbstractCanAddressingInformation.__init__(
                self=self.mock_addressing_information,
        ...
        mock_abstract_ai_init.assert_called_once_with(
            rx_physical_params=rx_physical_params,
            tx_physical_params=tx_physical_params,
            rx_functional_params=rx_functional_params,
            tx_functional_params=tx_functional_params,
    ...
    @pytest.mark.parametrize(
        "can_id, raw_frame_data, ai_data_bytes_number, decoded_can_id_params, decoded_data_ai_params, expected_output",
        [
            (
                Mock(),
                b"\xfe\xdc\xba\x98\x76\x54\x32\x10"[::-1],
                0,
                {
    ...
    def test_decode_frame_ai_params(
        self,
        mock_decode_can_id_ai_params,
        mock_decode_data_bytes_ai_params,
        can_id,
        raw_frame_data,
        ai_data_bytes_number,
        decoded_can_id_params,
        decoded_data_ai_params,
        expected_output,
    ):
        ...
        assert (
            AbstractCanAddressingInformation.decode_frame_ai_params(can_id=can_id, raw_frame_data=raw_frame_data)
            == expected_output
        )
    ...
    @pytest.mark.parametrize(
        "can_id, raw_frame_data, decoded_frame_ai_params, rx_physical_params, rx_functional_params",
        [
            (
                Mock(),
                b"\xfe\xdc\xba\x98\x76\x54\x32\x10",
    ...
    def test_is_input_packet__none(
        self, can_id, raw_frame_data, decoded_frame_ai_params, rx_physical_params, rx_functional_params
    ):
        ...
        assert (
            AbstractCanAddressingInformation.is_input_packet(
                self.mock_addressing_information, can_id=can_id, raw_frame_data=raw_frame_data
            )
            is None
        )
    ...
    def test_is_input_packet__none_from_exception(self):
        ...
        assert (
            AbstractCanAddressingInformation.is_input_packet(
                self.mock_addressing_information, can_id=mock_can_id, raw_frame_data=mock_raw_frame_data
            )
            is None
        )
        self.mock_addressing_information.decode_frame_ai_params.assert_called_once_with(
