...
class TestMixed11BitCanAddressingInformation:
    ...
    def test_validate_node_ai__inconsistent(
    ...
    def test_validate_node_ai__valid(
        self, rx_physical_params, tx_physical_params, rx_functional_params, tx_functional_params
    ):
        self.mock_addressing_information.rx_physical_params = rx_physical_params
        self.mock_addressing_information.tx_physical_params = tx_physical_params
    ...
    def test_validate_addressing_params__value_error(self, addressing_format):
        with pytest.raises(ValueError):
            Mixed11BitCanAddressingInformation.validate_addressing_params(
                addressing_type=Mock(), can_id=Mock(), addressing_format=addressing_format
            )

    @pytest.mark.parametrize(
        "unsupported_args",
        [{"target_address": 0x2C}, {"source_address": 0x96}, {"target_address": Mock(), "source_address": Mock()}],
    )
    def test_validate_addressing_params__inconsistent_arg(self, unsupported_args):
        with pytest.raises(UnusedArgumentError):
            Mixed11BitCanAddressingInformation.validate_addressing_params(
                addressing_type=Mock(), can_id=Mock(), address_extension=Mock(), **unsupported_args
            )
    ...
    def test_validate_addressing_params__inconsistent(
        self, mock_is_compatible_can_id, addressing_type, can_id, address_extension
    ):
        mock_is_compatible_can_id.return_value = False
        with pytest.raises(InconsistencyError):
            Mixed11BitCanAddressingInformation.validate_addressing_params(
                addressing_type=addressing_type, can_id=can_id, address_extension=address_extension
            )
    ...
    @patch(f"{SCRIPT_LOCATION}.Mixed11BitCanAddressingInformation.is_compatible_can_id")
    def test_validate_addressing_params__valid(
        self, mock_is_compatible_can_id, addressing_type, can_id, address_extension
    ):
        mock_is_compatible_can_id.return_value = True
        assert Mixed11BitCanAddressingInformation.validate_addressing_params(
            addressing_type=addressing_type, can_id=can_id, address_extension=address_extension
        ) == {
    ...
    def test_is_compatible_can_id(self, can_id, addressing_type):
        assert (
            Mixed11BitCanAddressingInformation.is_compatible_can_id(can_id, addressing_type)
...
class TestMixed29BitCanAddressingInformation:
    ...
    def test_validate_addressing_params__value_error(self, addressing_format):
        with pytest.raises(ValueError):
            Mixed29BitCanAddressingInformation.validate_addressing_params(
                addressing_type=Mock(), can_id=Mock(), addressing_format=addressing_format
            )
    ...
    def test_validate_addressing_params__inconsistent__missing_info(
        self, addressing_type, can_id, target_address, source_address, address_extension
    ):
    ...
    @pytest.mark.parametrize(
        "addressing_type, decoded_addressing_type, ta, decoded_ta, sa, decoded_sa, can_id, address_extension",
        [
    ...
    @pytest.mark.parametrize(
        "addressing_type, target_address, source_address, address_extension",
        [
            (Mock(), Mock(), Mock(), Mock()),
    ...
    def test_validate_addressing_params__valid_without_can_id(
        self, mock_encode_can_id, addressing_type, target_address, source_address, address_extension
    ):
        assert Mixed29BitCanAddressingInformation.validate_addressing_params(
            addressing_type=addressing_type,
            can_id=None,
            target_address=target_address,
            source_address=source_address,
    ...
    @pytest.mark.parametrize(
        "addressing_type, target_address, source_address, can_id, address_extension",
        [
    ...
    def test_validate_addressing_params__valid_with_can_id(
        self, mock_decode_can_id_ai_params, addressing_type, target_address, source_address, can_id, address_extension
    ):
    ...
    def test_encode_can_id__not_implemented(self, addressing_type, target_address, source_address):
        self.mock_validate_addressing_type.return_value = addressing_type
        with pytest.raises(NotImplementedError):
            Mixed29BitCanAddressingInformation.encode_can_id(
