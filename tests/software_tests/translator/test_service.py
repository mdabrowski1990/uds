...
class TestService:
    ...
    def setup_method(self):
        self.mock_service = Mock(spec=Service, NEGATIVE_RESPONSE_LENGTH=3)
        # patching
        self._patcher_deepcopy = patch(f"{SCRIPT_LOCATION}.deepcopy")
        self.mock_deepcopy = self._patcher_deepcopy.start()
        self._patcher_validate_message_continuation = patch(
            f"{SCRIPT_LOCATION}.AbstractConditionalDataRecord.validate_message_continuation"
        )
    ...
    @pytest.mark.parametrize(
        "request_sid, request_structure, response_structure",
        [
            (Mock(), Mock(), Mock()),
    ...
    @pytest.mark.parametrize(
        "request_sid, request_structure, response_structure, supported_nrc",
        [
            (Mock(), Mock(), Mock(), Mock()),
    ...
    def test_deepcopy(self, mock_init):
        ...
        self.mock_deepcopy.assert_has_calls(
            [
                call(self.mock_service.request_structure, memo=memo),
    ...
    def test_get_single_data_record_occurrence__valid__no_occurrence(self):
        mock_data_record = Mock(spec=AbstractDataRecord, is_reoccurring=False, min_occurrences=0, max_occurrences=1)
    ...
    @pytest.mark.parametrize(
        "data_record, value",
        [
            (Mock(spec=AbstractDataRecord, is_reoccurring=True, min_occurrences=0, max_occurrences=8), list(range(9))),
            (
                Mock(
                    spec=AbstractDataRecord,
                    is_reoccurring=True,
    ...
    def test_get_reoccurring_data_record_occurrences__valid__no_occurrences(self):
        mock_data_record = Mock(spec=AbstractDataRecord, is_reoccurring=True, min_occurrences=0, max_occurrences=None)
    ...
    def test_get_data_record_occurrences__single(
        self, mock_get_single_data_record_occurrence, mock_get_reoccurring_data_record_occurrences
    ):
        mock_data_record = Mock(spec=AbstractDataRecord, is_reoccurring=False)
        mock_value = MagicMock()
        assert (
            Service._get_data_record_occurrences(data_record=mock_data_record, value=mock_value)
            == mock_get_single_data_record_occurrence.return_value
        )
        mock_get_single_data_record_occurrence.assert_called_once_with(data_record=mock_data_record, value=mock_value)
        mock_get_reoccurring_data_record_occurrences.assert_not_called()

    @patch(f"{SCRIPT_LOCATION}.Service._get_reoccurring_data_record_occurrences")
    @patch(f"{SCRIPT_LOCATION}.Service._get_single_data_record_occurrence")
    def test_get_data_record_occurrences__multiple(
        self, mock_get_single_data_record_occurrence, mock_get_reoccurring_data_record_occurrences
    ):
        ...
        mock_get_reoccurring_data_record_occurrences.assert_called_once_with(
            data_record=mock_data_record, value=mock_value
        )
    ...
    @pytest.mark.parametrize(
        "message_structure, remaining_length",
        [
            ([Mock(spec=AbstractDataRecord, min_occurrences=8, length=5)], 40),
            (
                (
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                list(range(100)),
                [
                    Mock(spec=AbstractDataRecord, length=48, min_occurrences=1, max_occurrences=1),
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                list(range(4)),
                [
                    Mock(spec=AbstractDataRecord, length=40, min_occurrences=1, max_occurrences=1),
    ...
    @pytest.mark.parametrize(
        "payload, message_structure, message_continuation",
        [
            (
                (0xCA, 0xFE),
                [
    ...
    def test_decode_payload__valid__no_condition(self, payload, message_structure):
        assert Service._decode_payload(payload=payload, message_structure=message_structure) == tuple(
            data_record.get_occurrence_info.return_value for data_record in message_structure
        )
    ...
    def test_decode_payload__valid__remaining_length_2(self, mock_get_remaining_length, payload, message_structure):
        mock_get_remaining_length.side_effect = TypeError
        assert Service._decode_payload(payload=payload, message_structure=message_structure) == tuple(
            data_record.get_occurrence_info.return_value for data_record in message_structure[:1]
        )
    ...
    def test_decode_payload__valid__condition(self, payload, message_structure, message_continuation):
        self.mock_int_to_bytes.side_effect = [
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [Mock(spec=AbstractDataRecord, name="Param 1", length=8), Mock(name="Param 2", length=1)],
                {"Param 1": 0, "Param 2": 0},
            ),
        ],
    )
    @patch(f"{SCRIPT_LOCATION}.Service._get_data_record_occurrences")
    def test_encode_message__not_implemented(
        self, mock_get_data_record_occurrences, message_structure, data_records_values
    ):
        ...
        with pytest.raises(NotImplementedError):
            Service._encode_message(data_records_values=data_records_values, message_structure=message_structure)

    @pytest.mark.parametrize(
        "message_structure, data_records_values",
        [
            ([Mock(spec=AbstractDataRecord, name="Param 1", length=7)], {"Param 1": 0}),
            (
                [
                ...
                {
                    "Data Record - 1": [
                        {"Data Record - 1.1": 0, "Data Record - 1.2": 2},
                        0,
    ...
    @patch(f"{SCRIPT_LOCATION}.Service._get_data_record_occurrences")
    def test_encode_message__runtime_error(
        self, mock_get_data_record_occurrences, message_structure, data_records_values
    ):
        ...
        mock_get_data_record_occurrences.assert_has_calls(
            [call(data_record=dr, value=values_copy[dr.name]) for dr in message_structure], any_order=False
        )
        self.mock_int_to_bytes.assert_not_called()
    ...
    @pytest.mark.parametrize(
        "message_structure, data_records_values",
        [
            ([Mock(spec=AbstractDataRecord, name="Param 1", length=8)], {"Param 1": 0, "Param 2": 1}),
            (
    ...
    @patch(f"{SCRIPT_LOCATION}.Service._get_data_record_occurrences")
    def test_encode_message__value_error(
        self, mock_get_data_record_occurrences, message_structure, data_records_values
    ):
        values_copy = deepcopy(data_records_values)
        mock_get_data_record_occurrences.side_effect = self.get_data_record_occurrences
    ...
    @patch(f"{SCRIPT_LOCATION}.Service._get_data_record_occurrences")
    def test_encode_message__inconsistency_error(
        self, mock_get_data_record_occurrences, message_structure, data_records_values
    ):
        mock_get_data_record_occurrences.side_effect = self.get_data_record_occurrences
        for data_record in message_structure:
    ...
    @pytest.mark.parametrize(
        "message_structure, data_records_values, payload",
        [
            ([Mock(spec=AbstractDataRecord, name="Param 1", length=8)], {"Param 1": 0}, b"\x92\x8b\x7a\xdc"),
            (
                [Mock(spec=AbstractDataRecord, name="XYZ", length=24)],
    ...
    @patch(f"{SCRIPT_LOCATION}.Service._get_data_record_occurrences")
    def test_encode_message__valid__no_condition(
        self, mock_get_data_record_occurrences, message_structure, data_records_values, payload
    ):
        values_copy = deepcopy(data_records_values)
        ...
        for data_record in message_structure:
            data_record.name = data_record._extract_mock_name()
        assert Service._encode_message(
            data_records_values=data_records_values, message_structure=message_structure
        ) == bytearray(payload)
        mock_get_data_record_occurrences.assert_has_calls(
            [call(data_record=dr, value=values_copy[dr.name]) for dr in message_structure], any_order=False
        )
    ...
    @pytest.mark.parametrize(
        "message_structure, data_records_values, message_continuation, payload",
        [
            (
                [Mock(spec=AbstractDataRecord, name="PAram", length=8), Mock(spec=AbstractConditionalDataRecord)],
                {"PAram": 0xD0},
                [],
                b"\xd0",
            ),
    ...
    @patch(f"{SCRIPT_LOCATION}.Service._get_data_record_occurrences")
    def test_encode_message__valid__condition(
        self, mock_get_data_record_occurrences, message_structure, data_records_values, message_continuation, payload
    ):
        values_copy = deepcopy(data_records_values)
        self.mock_int_to_bytes.return_value = payload
        mock_get_data_record_occurrences.side_effect = self.get_data_record_occurrences
    ...
    @pytest.mark.parametrize(
        "message_structure, data_records_values, message_continuation_1, message_continuation_2, payload",
        [
            (
                [
                    Mock(spec=AbstractDataRecord, name="Data Record - 1", length=8),
            ...
            (
                ...
                {"A": 0xF0, "B1": 0xE1, "B2": 0xD2, "C": 0xC3, "D": 0xB4A5},
                [
                    Mock(spec=AbstractDataRecord, name="B1", length=8),
                    Mock(spec=AbstractDataRecord, name="B2", length=8),
                ],
    ...
    @patch(f"{SCRIPT_LOCATION}.Service._get_data_record_occurrences")
    def test_encode_message__valid__condition_after_condition(
        self,
        mock_get_data_record_occurrences,
        message_structure,
        data_records_values,
        message_continuation_1,
        message_continuation_2,
        payload,
    ):
        values_copy = deepcopy(data_records_values)
        self.mock_int_to_bytes.return_value = payload
    ...
    @patch(f"{SCRIPT_LOCATION}.Service._get_data_record_occurrences")
    def test_encode_message__valid__condition_in_condition(
        self, mock_get_data_record_occurrences, message_structure, data_records_values, message_continuation, payload
    ):
        self.mock_int_to_bytes.return_value = payload
        mock_get_data_record_occurrences.side_effect = self.get_data_record_occurrences
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                [Mock(spec=AbstractDataRecord, name="Param 1", length=8), Mock(spec=AbstractConditionalDataRecord)],
                {"Param 1": []},
                b"",
            ),
            (
    ...
    @patch(f"{SCRIPT_LOCATION}.Service._get_data_record_occurrences")
    def test_encode_message__valid__with_uncalled_condition(
        self, mock_get_data_record_occurrences, message_structure, data_records_values, payload
    ):
        self.mock_int_to_bytes.return_value = payload
        mock_get_data_record_occurrences.side_effect = self.get_data_record_occurrences
    ...
    def test_decode_request__valid(self, payload):
        ...
        assert (
            Service.decode_request(self.mock_service, payload) == self.mock_service._get_sid_info.return_value,
            *self.mock_service._decode_payload.return_value,
        )
        self.mock_validate_raw_bytes.assert_called_once_with(payload, allow_empty=False)
        self.mock_service._decode_payload.assert_called_once_with(
            payload=payload[1:], message_structure=self.mock_service.request_structure
        )
    ...
    def test_decode_positive_response__valid(self, payload):
        ...
        self.mock_validate_raw_bytes.assert_called_once_with(payload, allow_empty=False)
        self.mock_service._decode_payload.assert_called_once_with(
            payload=payload[1:], message_structure=self.mock_service.response_structure
        )
    ...
    def test_decode__request(self, payload):
        self.mock_service.request_sid = payload[0]
        assert Service.decode(self.mock_service, payload=payload) == self.mock_service.decode_request.return_value
    ...
    def test_decode__positive_response(self, payload):
        ...
        assert (
            Service.decode(self.mock_service, payload=payload)
            == self.mock_service.decode_positive_response.return_value
        )

    @pytest.mark.parametrize(
        "payload",
        [
            [ResponseSID.NegativeResponse, *range(100, 232)],
    ...
    def test_decode__negative_response(self, payload):
        assert (
            Service.decode(self.mock_service, payload=payload)
            == self.mock_service.decode_negative_response.return_value
        )

    # encode_request

    @pytest.mark.parametrize(
        "data_records_values, request_sid, payload_continuation",
        [
    ...
    def test_encode_request(self, data_records_values, request_sid, payload_continuation):
        self.mock_service.request_sid = request_sid
        self.mock_service._encode_message.return_value = payload_continuation
        assert (
            Service.encode_request(self.mock_service, data_records_values=data_records_values)
    ...
    @pytest.mark.parametrize(
        "data_records_values, response_sid, payload_continuation",
        [
            (MagicMock(), 0xA5, bytearray(b"\xf0\xe1\xd2\xc3\xb4\xa5\x96\x87")),
            ({"a": 1, "b": [{"zyx a": 94}, 0xFF]}, ResponseSID.ControlDTCSetting, bytearray([0xBE, 0xEF])),
        ],
    )
    def test_encode_positive_response(self, data_records_values, response_sid, payload_continuation):
        self.mock_service.response_sid = response_sid
        self.mock_service._encode_message.return_value = payload_continuation
        assert (
            Service.encode_positive_response(self.mock_service, data_records_values=data_records_values)
    ...
    def test_encode_negative_response__warning(self, nrc, sid):
        ...
        self.mock_service.supported_nrc = set()
        assert Service.encode_negative_response(self.mock_service, nrc) == bytearray(
            (ResponseSID.NegativeResponse, sid, nrc)
        )
        self.mock_warn.assert_called_once()
    ...
    def test_encode__inconsistency_error(self, data_records_values):
        with pytest.raises(InconsistencyError):
            Service.encode(
                self.mock_service, sid=None, rsid=ResponseSID.NegativeResponse, data_records_values=data_records_values
            )
    ...
    def test_encode__request(self, data_records_values):
        assert (
            Service.encode(
                self.mock_service, sid=self.mock_service.request_sid, data_records_values=data_records_values
            )
    ...
    def test_encode__positive_response(self, data_records_values):
        assert (
            Service.encode(
                self.mock_service, rsid=self.mock_service.response_sid, data_records_values=data_records_values
            )
