...
class TestSingleOccurrenceInfo:
    def setup_method(self):
        self.mock_info = Mock(spec=SingleOccurrence)
        # patching
    ...
    def test_getitem__valid(self):
        mock_name = Mock()
        assert SingleOccurrence.__getitem__(self.mock_info, mock_name) == self.mock_find_element.return_value
        self.mock_find_element.assert_called_once_with(self.mock_info.children, name=mock_name)

    def test_getitem__value_error(self):
        self.mock_find_element.return_value = None
        mock_name = Mock()
        with pytest.raises(ValueError):
            SingleOccurrence.__getitem__(self.mock_info, mock_name)
        self.mock_find_element.assert_called_once_with(self.mock_info.children, name=mock_name)
    ...
    def test_to_dict(self, children_count):
        ...
        assert SingleOccurrence.to_dict(self.mock_info) == SingleOccurrenceDict(
            name=self.mock_info.name,
            ...
            children=tuple([child.to_dict.return_value for child in self.mock_info.children]),
        )


class TestMultipleOccurrencesInfo:
    def setup_method(self):
        self.mock_info = Mock(spec=MultipleOccurrences)
        # patching
        self._patcher_find_element = patch(f"{SCRIPT_LOCATION}.find_element")
        self.mock_find_element = self._patcher_find_element.start()
    ...
    def test_getitem__value_error(self):
        self.mock_find_element.return_value = None
        mock_name = Mock()
        self.mock_info.children = [Mock()] * 10
        with pytest.raises(ValueError):
            MultipleOccurrences.__getitem__(self.mock_info, mock_name)
        self.mock_find_element.assert_called_once_with(self.mock_info.children[0], name=mock_name)

    # to_dict

    @pytest.mark.parametrize(
        "children_count, occurrence_count",
        [
            (0, 1),
            (3, 0),
            (2, 3),
    ...
    def test_to_dict(self, children_count, occurrence_count):
        ...
        assert MultipleOccurrences.to_dict(self.mock_info) == MultipleOccurrencesDict(
            name=self.mock_info.name,
            length=self.mock_info.length,
            raw_value=self.mock_info.raw_value,
            physical_value=self.mock_info.physical_value,
            ...
            children=tuple(
                [
                    tuple([child.to_dict.return_value for child in occurrence_data])
...
class TestAbstractDataRecord:
    ...
    def test_getitem__valid(self):
        mock_name = Mock()
        assert AbstractDataRecord.__getitem__(self.mock_data_record, mock_name) == self.mock_find_element.return_value
        self.mock_find_element.assert_called_once_with(self.mock_data_record.children, name=mock_name)

    def test_getitem__value_error(self):
        self.mock_find_element.return_value = None
        mock_name = Mock()
        with pytest.raises(ValueError):
            AbstractDataRecord.__getitem__(self.mock_data_record, mock_name)
        self.mock_find_element.assert_called_once_with(self.mock_data_record.children, name=mock_name)
    ...
    def test_length__get(self):
        ...
        assert (
            AbstractDataRecord.length.fget(self.mock_data_record) == self.mock_data_record._AbstractDataRecord__length
        )
    ...
    def test_children__get(self):
        ...
        assert (
            AbstractDataRecord.children.fget(self.mock_data_record)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                8,
                [
                    Mock(spec=AbstractDataRecord, fixed_total_length=True, length=4, min_occurrences=2),
            ...
            (
                16,
                [
                    Mock(spec=AbstractDataRecord, fixed_total_length=True, length=8, min_occurrences=1),
                    Mock(spec=AbstractDataRecord, fixed_total_length=True, length=4, min_occurrences=1),
    ...
    def test_min_occurrences__get(self):
        ...
        assert (
            AbstractDataRecord.min_occurrences.fget(self.mock_data_record)
            == self.mock_data_record._AbstractDataRecord__min_occurrences
        )
    ...
    def test_max_occurrences__get(self):
        ...
        assert (
            AbstractDataRecord.max_occurrences.fget(self.mock_data_record)
            == self.mock_data_record._AbstractDataRecord__max_occurrences
        )
    ...
    def test_unit__get(self):
        ...
        assert AbstractDataRecord.unit.fget(self.mock_data_record) == self.mock_data_record._AbstractDataRecord__unit
    ...
    def test_enforce_reoccurring__get(self):
        ...
        assert (
            AbstractDataRecord.enforce_reoccurring.fget(self.mock_data_record)
    ...
    @pytest.mark.parametrize(
        "min_occurrences, max_occurrences, expect_value",
        [
            (1, None, False),
            (3, 3, True),
            (1, 1, True),
    ...
    @pytest.mark.parametrize(
        "children, raw_value, expected_children_values",
        [
            ([Mock(length=4, min_occurrences=1), Mock(length=4, min_occurrences=1)], 0x5A, [(0x5,), (0xA,)]),
            (
                [
                    Mock(length=2, min_occurrences=5),
                    Mock(length=4, min_occurrences=1),
                    Mock(length=10, min_occurrences=1),
                ],
                0xA555FE,
                [(0b10, 0b10, 0b01, 0b01, 0b01), (0b0101,), (0x1FE,)],
            ),
            ([Mock(length=1, min_occurrences=8)], 0xAA, [(1, 0) * 4]),
        ],
    )
    ...
    def test_get_occurrence_info__single_occurrence(self):
        ...
        assert isinstance(output, SingleOccurrence)
    ...
    def test_get_occurrence_info__multiple_occurrences(self, raw_values):
        ...
        assert isinstance(output, MultipleOccurrences)
        assert output.name == self.mock_data_record.name
        ...
        self.mock_data_record.get_children_occurrence_info.assert_has_calls(
            [call(raw_value) for raw_value in raw_values]
        )
    ...
    @pytest.mark.parametrize(
        "raw_values, min_occurrences, max_occurrences",
        [
            (range(11), 0, 10),
            ([Mock()], 2, None),
        ],
    ...
    @patch(f"{SCRIPT_LOCATION}.isinstance")
    def test_get_raw_value_from_children__value_error__children_names(
        self, mock_isinstance, children_names, children_values
    ):
    ...
    @pytest.mark.parametrize(
        "children, children_values, raw_value",
        [
            (
                (
                    Mock(spec=AbstractDataRecord, length=2, name="Parameter A"),
    ...
    def test_get_raw_value_from_children__valid__raw_values(self, children, children_values, raw_value):
        ...
        self.mock_data_record.children = children
        self.mock_data_record.length = sum([child.length for child in children])
        assert AbstractDataRecord.get_raw_value_from_children(self.mock_data_record, children_values) == raw_value

    @pytest.mark.parametrize(
        "children, children_values, children_raw_values_mapping, raw_value",
        [
            (
                (
                    Mock(spec=AbstractDataRecord, length=2, name="Parameter A"),
    ...
    def test_get_raw_value_from_children__valid__children_values(
        self, children, children_values, children_raw_values_mapping, raw_value
    ):
        for child in children:
            child.name = child._extract_mock_name()
