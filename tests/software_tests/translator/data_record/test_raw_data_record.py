...
class TestRawDataRecord:
    ...
    def test_init__mandatory_args(self, mock_abstract_data_record_class, name, length):
        ...
        mock_abstract_data_record_class.assert_called_once_with(
            name=name,
            length=length,
            children=tuple(),
            unit=None,
    ...
    def test_init__all_args(
        self,
        mock_abstract_data_record_class,
        name,
        length,
        children,
        unit,
    ...
    @pytest.mark.parametrize("value", [0, 0xFF, Mock()])
...
class TestRawDataRecordIntegration:
    ...
    def setup_class(self):
        self.dtc = RawDataRecord(name="DTC", length=24, min_occurrences=0, max_occurrences=1)
