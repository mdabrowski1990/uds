...
class TestConfigurableTranslator:
    ...
    @pytest.mark.parametrize(
        "services, diagnostic_session_type_mapping, reset_type_mapping, report_type_mapping,"
    ...
    def test_init__all_args(
        ...
        assert (
            ConfigurableTranslator.__init__(
                ...
                did_data_mapping=did_data_mapping,
            )
            is None
        )
    ...
    def test_did_data_mapping__get(self):
        ...
        assert (
            ConfigurableTranslator.did_data_mapping.fget(self.mock_translator)
    ...
    def test_did_records__get(self):
        assert ConfigurableTranslator._ConfigurableTranslator__did_records.fget(
            self.mock_translator
    ...
    def test_dtc_snapshot_records__get(self):
        ...
        dtc_snapshot_records = ConfigurableTranslator._ConfigurableTranslator__dtc_snapshot_records.fget(
            self.mock_translator
        )
        assert isinstance(dtc_snapshot_records, tuple)
        assert all(
            item == OPTIONAL_DTC_SNAPSHOT_RECORDS_NUMBERS_LIST[i] for i, item in enumerate(dtc_snapshot_records[::3])
        )
        assert all(item == DID_COUNT_RECORDS[i] for i, item in enumerate(dtc_snapshot_records[1::3]))
        assert all(
            item == self.mock_translator._ConfigurableTranslator__did_records[i]
    ...
    def test_dtc_stored_data_records__get(self):
        self.mock_translator._ConfigurableTranslator__did_records = [Mock()] * REPEATED_DATA_RECORDS_NUMBER
        dtc_stored_data_records = ConfigurableTranslator._ConfigurableTranslator__dtc_stored_data_records.fget(
            self.mock_translator
        )
        assert isinstance(dtc_stored_data_records, tuple)
        assert all(
            item == DTC_STORED_DATA_RECORD_NUMBERS_LIST[i] for i, item in enumerate(dtc_stored_data_records[::4])
        )
        assert all(item == DTCS_AND_STATUSES_LIST[i] for i, item in enumerate(dtc_stored_data_records[1::4]))
        assert all(item == DID_COUNT_RECORDS[i] for i, item in enumerate(dtc_stored_data_records[2::4]))
        assert all(
            item == self.mock_translator._ConfigurableTranslator__did_records[i]
    ...
    def test_conditional_control_state__get(self):
        assert (
            ConfigurableTranslator._ConfigurableTranslator__conditional_control_state.fget(self.mock_translator)
            == self.mock_translator._ConfigurableTranslator__get_did_data.return_value
        )
    ...
    def test_conditional_optional_control_enable_mask__get(self):
        assert (
            ConfigurableTranslator._ConfigurableTranslator__conditional_optional_control_enable_mask.fget(
                self.mock_translator
            )
            == self.mock_translator._ConfigurableTranslator__get_did_data_mask.return_value
        )
    ...
    def test_conditional_activated_events__get(self):
        assert (
            ConfigurableTranslator._ConfigurableTranslator__conditional_activated_events.fget(self.mock_translator)
    ...
    def test_get_did__valid(self, name, optional):
        assert (
            ConfigurableTranslator._ConfigurableTranslator__get_did(self.mock_translator, name=name, optional=optional)
            == self.mock_mapping_data_record.return_value
        )
    ...
    def test_get_did_data_mask(self, name, optional):
        assert (
            ConfigurableTranslator._ConfigurableTranslator__get_did_data_mask(
                self.mock_translator, name=name, optional=optional
            )
            == self.mock_conditional_formula_data_record.return_value
        )
    ...
    def test_get_did_records_formula(self):
        ...
        mock_did_count = Mock()
        formula = ConfigurableTranslator._ConfigurableTranslator__get_did_records_formula(
            self.mock_translator, mock_record_number
        )
    ...
    def test_get_did_record(self, did_count, record_number):
        assert (
            ConfigurableTranslator._ConfigurableTranslator__get_did_record(
                self.mock_translator, did_count=did_count, record_number=record_number
            )
            == (
    ...
    def test_get_input_output_control_by_identifier_request(self):
        ...
        self.mock_translator._ConfigurableTranslator__conditional_optional_control_enable_mask.get_message_continuation.assert_called_once_with(
            mock_did
        )
    ...
    def test_get_event_window_time(self, event_number):
        assert (
            ConfigurableTranslator._ConfigurableTranslator__get_event_window_time(self.mock_translator, event_number)
    ...
    def test_get_activated_events__only_mandatory(
        ...
        assert (
            ConfigurableTranslator._ConfigurableTranslator__get_activated_events(
                self.mock_translator, number_of_activated_events
            )
            == (
        ...
        self.mock_conditional_mapping_data_record.assert_has_calls(
            number_of_activated_events
            * [
                call(
                    mapping={
                        0x01: (
                            self.mock_translator._ConfigurableTranslator__get_event_window_time.return_value,
    ...
    def test_get_event_type_of_active_event(self, event_number):
        assert (
            ConfigurableTranslator._ConfigurableTranslator__get_event_type_of_active_event(
                self.mock_translator, event_number
            )
    ...
    def test_get_event_type_record_09_continuation__none(self, event_number):
        self.mock_translator.services_mapping = {
            RequestSID.ResponseOnEvent: MagicMock(response_structure=[MagicMock(), MagicMock(mapping={})]),
        }
        assert (
            ConfigurableTranslator._ConfigurableTranslator__get_event_type_record_09_continuation(
                self.mock_translator, event_number=event_number
            )
            is None
        )
        self.mock_deepcopy.assert_not_called()
    ...
    def test_get_event_type_record_09_continuation__valid(self, event_number):
        ...
        self.mock_deepcopy.return_value = Mock(
            mapping={
                0x04: [Mock(), Mock()],
                0x09: [Mock()],
                0x12: [Mock(), Mock(), Mock()],
...
class TestConfigurableTranslatorIntegration:
    ...
    def test_configuration_1(self, configurable_translator_1):
        ...
        assert (
            configurable_translator_1.timing_parameter_access_type_mapping == self.timing_parameter_access_type_mapping
        )
        ...
        # unchanged base
        assert (
            self.minimalistic_translator.services_mapping[RequestSID.DiagnosticSessionControl]
            == DIAGNOSTIC_SESSION_CONTROL
        )
        assert (
            DIAGNOSTIC_SESSION_CONTROL.request_structure[0].children[1].values_mapping
            != configurable_translator_1.diagnostic_session_type_mapping
        )
        assert (
            DIAGNOSTIC_SESSION_CONTROL.response_structure[0].children[1].values_mapping
            != configurable_translator_1.diagnostic_session_type_mapping
        )
        assert self.minimalistic_translator.services_mapping[RequestSID.TesterPresent] == TESTER_PRESENT
        assert (
            TESTER_PRESENT.request_structure[0].children[1].values_mapping
            != configurable_translator_1.zero_subfunction_mapping
        )
        assert (
            TESTER_PRESENT.response_structure[0].children[1].values_mapping
            != configurable_translator_1.zero_subfunction_mapping
        )
        assert (
            self.minimalistic_translator.services_mapping[RequestSID.AccessTimingParameter]
            == ACCESS_TIMING_PARAMETER_2013
        )
        assert (
            ACCESS_TIMING_PARAMETER_2013.request_structure[0].children[1].values_mapping
            != configurable_translator_1.timing_parameter_access_type_mapping
        )
        assert (
            ACCESS_TIMING_PARAMETER_2013.response_structure[0].children[1].values_mapping
            != configurable_translator_1.timing_parameter_access_type_mapping
        )
    ...
    def test_configuration_2(self, configurable_translator_2):
        ...
        assert (
            BASE_TRANSLATOR.services_mapping[RequestSID.DynamicallyDefineDataIdentifier]
            == DYNAMICALLY_DEFINE_DATA_IDENTIFIER
        )
        assert (
            DYNAMICALLY_DEFINE_DATA_IDENTIFIER.request_structure[0].children[1].values_mapping
            == DEFINITION_TYPE_MAPPING
        )
        assert (
            DYNAMICALLY_DEFINE_DATA_IDENTIFIER.response_structure[0].children[1].values_mapping
            == DEFINITION_TYPE_MAPPING
        )
        ...
        assert (
            BASE_TRANSLATOR.services_mapping[RequestSID.ReadScalingDataByIdentifier] == READ_SCALING_DATA_BY_IDENTIFIER
        )
        ...
        assert (
            BASE_TRANSLATOR.services_mapping[RequestSID.InputOutputControlByIdentifier]
            == INPUT_OUTPUT_CONTROL_BY_IDENTIFIER
        )
        ...
        assert (
            BASE_TRANSLATOR.services_mapping[RequestSID.ReadDataByPeriodicIdentifier]
            == READ_DATA_BY_PERIODIC_IDENTIFIER
        )
