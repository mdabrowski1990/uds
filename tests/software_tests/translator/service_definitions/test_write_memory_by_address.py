...
class TestWriteMemoryByAddressIntegration:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [0x3D, 0x24, 0x80, 0x5D, 0x20, 0xA9, 0x00, 0x05, 0xAF, 0x31, 0xD3, 0xBB, 0x31],
                (
                    {
                        "children": (),
                        "length": 8,
    ...
    def test_encode(self, data_records_values, sid, rsid, payload):
        assert WRITE_MEMORY_BY_ADDRESS.encode(data_records_values=data_records_values, sid=sid, rsid=rsid) == payload
