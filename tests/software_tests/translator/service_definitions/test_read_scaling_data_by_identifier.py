...
class TestReadScalingDataByIdentifier2020Integration:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                [0x64, 0x01, 0x00, 0xA0, 0x4A, 0xA0, 0x0E, 0x92, 0x00, 0x00, 0x02, 0x10, 0x05],
                (
                    {
                        "children": (),
                        "length": 8,
    ...
    def test_encode(self, data_records_values, sid, rsid, payload):
        assert (
            READ_SCALING_DATA_BY_IDENTIFIER_2020.encode(data_records_values=data_records_values, sid=sid, rsid=rsid)
            == payload
        )
...
class TestReadScalingDataByIdentifier2013Integration:
            ...
            (
                {
                    ...
                    "scalingByte#3": 0x92,
                    "scalingByteExtension#3": {"formulaIdentifier": 0x03},
                    "C0#3": 0x2345,
                    "C1#3": 0xFEDC,
                    "scalingByte#4": 0x8C,
                    "scalingByte#5": 0x78,
    ...
    def test_encode(self, data_records_values, sid, rsid, payload):
        assert (
            READ_SCALING_DATA_BY_IDENTIFIER_2013.encode(data_records_values=data_records_values, sid=sid, rsid=rsid)
            == payload
        )
