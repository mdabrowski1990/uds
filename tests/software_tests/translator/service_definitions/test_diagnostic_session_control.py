...
class TestDiagnosticSessionControlIntegration:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {"SubFunction": {"suppressPosRspMsgIndicationBit": True, "diagnosticSessionType": 0x02}},
    ...
    def test_encode(self, data_records_values, sid, rsid, payload):
        assert DIAGNOSTIC_SESSION_CONTROL.encode(data_records_values=data_records_values, sid=sid, rsid=rsid) == payload
