...
class TestControlDTCSettingIntegration:
    ...
    def test_decode(self, payload, decoded_message):
        output = CONTROL_DTC_SETTING.decode(payload)
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                {"SubFunction": {"suppressPosRspMsgIndicationBit": True, "DTCSettingType": 0x02}},
    ...
    def test_encode(self, data_records_values, sid, rsid, payload):
        assert CONTROL_DTC_SETTING.encode(data_records_values=data_records_values, sid=sid, rsid=rsid) == payload
