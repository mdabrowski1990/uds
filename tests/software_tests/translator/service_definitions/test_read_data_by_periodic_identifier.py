...
class TestReadDataByPeriodicIdentifierIntegration:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ({}, None, ResponseSID.ReadDataByPeriodicIdentifier, bytearray([0x6A])),
            (
                {"Periodic DID": 0xC8, "data": (0xF0, 0xE1, 0xD2, 0xC3, 0xB4, 0xA5, 0x96, 0x87)},
                None,
    ...
    def test_encode(self, data_records_values, sid, rsid, payload):
        assert (
            READ_DATA_BY_PERIODIC_IDENTIFIER.encode(data_records_values=data_records_values, sid=sid, rsid=rsid)
            == payload
        )
